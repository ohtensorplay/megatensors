...
from .utils._mega_uris import _looks_like_mega_url
...
def _parse_bucket_uri(path: str) -> MegaUri:
    """Parse a bucket path into a MegaUri.

    Accepts:
    - `mega://buckets/namespace/name(/path/in/repo)` URIs,
    - MEGA web URLs such as `https://mega.tensorplay.cn/buckets/namespace/name(/tree/path)`,
    - plain `namespace/name(/path/in/repo)` paths.
    """
    if path.startswith(constants.MEGA_PROTOCOL) or _looks_like_mega_url(path):
        # Don't use 'if is_mega_uri(...)' here as we prefer 'parse_mega_uri(...)' to raise the exact error message.
        parsed = parse_mega_uri(path)
        if not parsed.is_bucket:
            raise ValueError(f"Invalid bucket path: {path}. Must be a bucket URI (mega://buckets/...).")
        return parsed
    parts = path.split("/", 2)
    ...
    prefix = "/".join(parts[2:])
    return MegaUri(type="bucket", id=bucket_id, path_in_repo=prefix)


def _is_bucket_path(path: str) -> bool:
    """Check if a path is a bucket path.

    Do not raise if the path is not a mega:// URI.
    Raise if the path is a mega:// URI but with an incorrect format.
    """
    if not path.startswith(constants.MEGA_PROTOCOL):
        return False
    return parse_mega_uri(path).is_bucket
...
def _list_remote_files(api: "MegaApi", bucket_id: str, prefix: str) -> Iterator[tuple[str, int, float, Any]]:
...
def _compute_sync_plan(
    ...
    if not is_upload and not is_download:
        raise ValueError("One of source or dest must be a bucket path (mega://buckets/...) and the other must be local.")
...
def _execute_plan(plan: SyncPlan, api: "MegaApi", verbose: bool = False, status: Any | None = None) -> None:
    """Execute a sync plan."""
    ...
    *,
    ...
    Args:
        ...
        dest (`str`, *optional*):
            ...
            Required unless using ``apply``.
        api ([`MegaApi`]):
            The MegaApi instance to use for API calls.
    ...
    if not source_is_bucket and not dest_is_bucket:
        raise ValueError("One of source or dest must be a bucket path (mega://buckets/...).")
