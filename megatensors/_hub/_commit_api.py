...
from .file_download import mega_hub_url
...
if TYPE_CHECKING:
    from .mega_api import CommitInfo, RepoFile
...
# Max is 1,000 per request on the Hub for MegaApi.get_paths_info
# Otherwise we get:
# MegaHubHTTPError: 413 Client Error: Payload Too Large for url: https://mega.tensorplay.cn/api/datasets/xxx (Request ID: xxx)\n\ntoo many parameters
# See https://github.com/huggingface/megatensors._hub/issues/1503
...
class CommitOperationCopy:
    ...
    [storage region](https://mega.tensorplay.cn/docs/hub/storage-regions); copying across regions is not supported.
...
def _fetch_files_to_copy(
    ...
    Raises:
        ...
        [`ValueError`](https://docs.python.org/3/library/exceptions.html#ValueError)
            If the Hub API response is improperly formatted.
    """
    from .mega_api import MegaApi, RepoFolder

    mega_api = MegaApi(endpoint=endpoint, headers=headers)
    copies = list(copies)
    ...
    for batch in chunk_iterable(dest_paths, FETCH_LFS_BATCH_SIZE):
        dest_repo_files = mega_api.get_paths_info(
            repo_id=repo_id,
            paths=list(batch),
            revision=revision,
            repo_type=repo_type,
    ...
    for (src_repo_id_key, src_repo_type_key, src_revision_key), group in groupby(
        ...
        for paths_batch in chunk_iterable(src_paths, FETCH_LFS_BATCH_SIZE):
            src_repo_files = mega_api.get_paths_info(
                repo_id=eff_repo_id,
...
def _prepare_commit_payload(
    ...
    For more information, see:
        - https://github.com/huggingface/megatensors._hub/issues/1085#issuecomment-1265208073
        - http://ndjson.org/
    """
...
def _send_commit(
    *,
    ...
    # See https://github.com/huggingface/megatensors._hub/issues/1085#issuecomment-1265208073
    ...
    mega_raise_for_status(response, endpoint_name="commit")

    commit_data = response.json()
    from .mega_api import CommitInfo
