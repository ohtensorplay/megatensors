...
from .mega_api import DEFAULT_IGNORE_PATTERNS, CommitInfo, CommitOperationAdd, MegaApi
...
class CommitScheduler:
    ...
    with the `stop` method. Checkout the [upload guide](https://mega.tensorplay.cn/docs/megatensors._hub/guides/upload#scheduled-uploads)
    ...
    Args:
        ...
        hf_api (`MegaApi`, *optional*):
            Source-compatible spelling for `mega_api`.
        mega_api (`MegaApi`, *optional*):
            The [`MegaApi`] client to use to commit to the Hub. Can be set with custom settings (user agent, token,...).

    Example:
    ```py
    >>> from pathlib import Path
    >>> from megatensors._hub import CommitScheduler
    ...
    def __init__(
        ...
        *,
        ...
        hf_api: Optional["MegaApi"] = None,
        mega_api: Optional["MegaApi"] = None,
    ) -> None:
        if hf_api is not None and mega_api is not None:
            raise ValueError("Pass only one of 'hf_api' or 'mega_api'.")
        self.api = hf_api or mega_api or MegaApi(token=token)
    ...
    def push_to_hub(self) -> CommitInfo | None:
        ...
        out our [integration guide](https://mega.tensorplay.cn/docs/megatensors._hub/main/en/guides/upload#scheduled-uploads).
        """
