...
def _get_duckdb_connection(
    ...
    try:
        ...
        if duckdb_binary is None:
            raise ImportError(
                "DuckDB is required for `mega datasets sql`. Install the Python package with `pip install duckdb` or "
...
def _build_duckdb_secret_statements(token: str | bool | None) -> list[str]:
    ...
    return [
        f"CREATE OR REPLACE SECRET mega_hub_token (TYPE HTTP, BEARER_TOKEN '{escaped_token}', SCOPE '{escaped_endpoint}')",
    ]
