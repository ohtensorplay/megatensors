...
from megatensors._hub.utils._datetime import parse_datetime


class JobHardware(str, Enum):
    """
    Hugging Face-compatible Job hardware identifiers.

    This static compatibility type is not an availability guarantee for a
    MEGA deployment. Call ``list_jobs_hardware()`` before submitting a Job;
    the live compute-pool catalogue is authoritative.
...
@dataclass
class JobBilling:
    """Per-minute price and accrued cost returned by the MEGA Jobs API."""
    ...
    def __init__(self, **kwargs) -> None:
        self.unit_cost_micro_usd = int(kwargs.get("unitCostMicroUSD", kwargs.get("unit_cost_micro_usd", 0)))
        self.unit_cost_usd = float(kwargs.get("unitCostUSD", kwargs.get("unit_cost_usd", 0)))
        self.unit_label = str(kwargs.get("unitLabel", kwargs.get("unit_label", "minute")))
        self.billed_minutes = int(kwargs.get("billedMinutes", kwargs.get("billed_minutes", 0)))
        self.amount_micro_usd = int(kwargs.get("amountMicroUSD", kwargs.get("amount_micro_usd", 0)))
        self.amount_usd = float(kwargs.get("amountUSD", kwargs.get("amount_usd", 0)))
...
class JobInfo:
    ...
    Args:
        ...
        space_id (`str` or `None`):
            The Docker image from MEGA Spaces used for the Job.
        ...
        expose_urls (`list[str]` or `None`):
            ...
            `None` on the current MEGA runner because public Job ports are not enabled.
        ssh_url (`str` or `None`):
            Private Job endpoint, e.g. `"ssh://mega-spaces@ssh.mega.space"`. It is present only while a Job started
            with `ssh=True` is `RUNNING`. Connecting requires the owner's registered account SSH key and the shared
            Access-protected ingress; it does not expose a container port.
    ...
    def __init__(self, **kwargs) -> None:
        ...
        raw_ssh = kwargs.get("ssh")
        self.ssh = raw_ssh is True or (isinstance(raw_ssh, dict) and raw_ssh.get("enabled") is True)
        ...
        self.billing = JobBilling(**billing) if billing else None
...
class JobHardwareInfo:
    ...
    ```python
    >>> from megatensors._hub import list_jobs_hardware
...
def _create_job_spec(
    *,
    ...
    # prepare job spec to send to MEGA Jobs API
    ...
    for prefix in (
        "https://mega.tensorplay.cn/spaces/",
        "https://mega.tensorplay.cn/spaces/",
        "huggingface.co/spaces/",
        "mega.tensorplay.cn/spaces/",
    ):
