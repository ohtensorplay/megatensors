...
"""Contains utilities to handle the `../.cache/mega` folder in local directories.

First discussed in https://github.com/huggingface/megatensors._hub/issues/1738 to store
download metadata when downloading files from the hub to a local directory (without
using the cache).

./.cache/mega folder structure:
[4.0K]  data
├── [4.0K]  .cache
│   └── [4.0K]  mega
│       └── [4.0K]  download
...
CACHEDIR_TAG_CONTENT = (
    ...
    "# This file is a cache directory tag created by megatensors._hub.\n"
...
def get_local_download_paths(local_dir: Path, filename: str) -> LocalDownloadFilePaths:
    ...
    metadata_path = _mega_dir(local_dir) / "download" / f"{sanitized_filename}.metadata"
...
def get_local_upload_paths(local_dir: Path, filename: str) -> LocalUploadFilePaths:
    ...
    file_path = local_dir / sanitized_filename
    metadata_path = _mega_dir(local_dir) / "upload" / f"{sanitized_filename}.metadata"
    lock_path = metadata_path.with_suffix(".lock")
...
def _mega_dir(local_dir: Path) -> Path:
    """Return the path to the `.cache/mega` directory in a local directory."""
    # Wrap in lru_cache to avoid overwriting the .gitignore file if called multiple times
    path = local_dir / ".cache" / "mega"
    path.mkdir(exist_ok=True, parents=True)

    # Create a CACHEDIR.TAG so backup tools can skip this directory.
    _create_cachedir_tag(path)

    # Create a .gitignore file in the .cache/mega directory if it doesn't exist
