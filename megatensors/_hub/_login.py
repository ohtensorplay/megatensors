...
from .errors import DeviceCodeError, LoginError, MegaHubHTTPError
from .utils import (
    ANSI,
    Spinner,
    get_token,
...
def login(
    ...
    *,
    ...
    is logged in and the access token will be available across all `megatensors._hub`
    components. If `token` is not provided, a browser-based OAuth flow is used to
    authenticate: the authorization URL is opened in the default browser, and the token
    is retrieved and saved after approval.
    In a terminal, you can also choose to paste an existing access token instead.

    To log in from outside of a script, one can also use `mega auth login` which is
...
def logout(token_name: str | None = None) -> None:
    ...
    if not token_name:
        ...
        for file_path in (constants.MEGA_TOKEN_PATH, constants.MEGA_STORED_TOKENS_PATH):
            try:
    ...
    if _get_token_from_google_colab() is not None:
        raise OSError(
            ...
            "To log out, you must unset the `MEGA_TOKEN` secret in your Colab settings."
        )
    if _get_token_from_environment() is not None:
        raise OSError(
            "Token has been deleted from your machine but you are still logged in.\n"
            "To log out, you must clear the `MEGA_TOKEN` environment variable."
        )
...
def auth_switch(token_name: str, add_to_git_credential: bool = False) -> None:
    ...
    if not token:
        raise ValueError(f"Access token {token_name} not found in {constants.MEGA_STORED_TOKENS_PATH}")
    # Write token to the active token file.
    ...
    if token_from_environment is not None and token_from_environment != token:
        logger.warning(
            "The environment variable `MEGA_TOKEN` is set and will override the access token you've just switched to."
        )
...
def auth_list() -> None:
    ...
    if not stored_tokens:
        if _get_token_from_environment():
            ...
            logger.warning("Note: Environment variable `MEGA_TOKEN` is set and is the current active token.")
        else:
    ...
    for token_name, fields in stored_tokens.items():
        token = fields.get("mega_token", "<not set>")
    ...
    if _get_token_from_environment():
        logger.warning(
            "\nNote: Environment variable `MEGA_TOKEN` is set and is the current active token independently from the stored tokens listed above."
        )
    elif current_token_name is None:
        logger.warning(
            "\nNote: No active token is set and no environment variable `MEGA_TOKEN` is found. Use `mega auth login` to log in."
        )
...
def _device_code_login() -> None:
    ...
    with Spinner("Opening browser..."):
        ...
        browser_opened = _open_browser(login_url)

    if not browser_opened:
        print(f"\n    Open this URL in your browser:\n        {login_url}\n")
        # A complete verification URI already contains the code. Only print it separately
        # when the authorization server did not provide a pre-filled URL.
        if login_url == device_info["verification_uri"]:
            print(f"    Enter the code: {device_info['user_code']}\n")
...
def _open_browser(url: str) -> bool:
    """Open the device authorization URL, falling back to printed instructions."""
    try:
...
def _save_oauth_token(response: OAuthTokenResponse) -> tuple[str, str]:
    ...
    if (
        ...
        raise LoginError("The authentication service returned an invalid response. Please try again.")
    expires_at = None
    if expires_in is not None:
        ...
        try:
            ...
            raise LoginError("The authentication service returned an invalid response. Please try again.") from error
        if expires_in_seconds <= 0:
            raise LoginError("The authentication service returned an invalid response. Please try again.")
        expires_at = int(time.time()) + expires_in_seconds
    return _validate_and_save_token(
        access_token,
...
def interpreter_login(*, skip_if_logged_in: bool = True) -> None:
    """
    Displays a prompt to log in to MEGA Hub and stores the token.
    ...
    Args:
        skip_if_logged_in (`bool`, defaults to `True`):
            ...
            Set to `False` to force re-login. In CLI, use `--force` instead.
    """
    if skip_if_logged_in and _is_logged_in():
        logger.info("User is already logged in. Use `mega auth login --force` to force re-login.")
        return
...
def notebook_login(*, skip_if_logged_in: bool = True) -> None:
    ...
    try:
        ...
        display(HTML(f"<center><b style='color: red;'>Login failed: {html.escape(str(error))}</b></center>"))
        return
    ...
    display(
        HTML(
            '<center><h2 style="font-family: sans-serif;">MEGA</h2>'
    ...
    try:
        ...
        logger.debug("Could not complete notebook login.", exc_info=True)
        display(HTML("<center><b style='color: red;'>Login failed. Please try again.</b></center>"))
        return

    message = f"Login successful. Logged in as <b>{html.escape(username)}</b>."
    if note := _expiration_note(response):
...
def _is_logged_in() -> bool:
    """Check login state without exposing refresh, OIDC, or local-storage failures."""
    try:
        return get_token() is not None
    except LoginError:
        raise
    except Exception as error:
        raise LoginError("Could not check existing login credentials. Please try again.") from error
...
def _validate_and_save_token(
    ...
    """
    from .mega_api import MegaApi  # avoid circular import
    ...
    try:
        token_info = MegaApi(endpoint=constants.ENDPOINT).whoami(token)
    except MegaHubHTTPError as error:
        if error.response.status_code == 401:
            raise LoginError("Access token is invalid.") from error
        raise LoginError(
        ...
        raise LoginError("Could not validate the access token. Please try again.") from error
    ...
    try:
        ...
        if not isinstance(username, str) or not username or not isinstance(token_name, str) or not token_name:
            raise TypeError
    ...
    try:
        ...
        raise LoginError("Could not save login credentials on this machine.") from error
    if _get_token_from_environment():
        logger.warning("A token from the environment remains active and overrides the token just saved.")
...
def _logout_from_token(token_name: str) -> None:
    ...
    if fields.get("mega_token") == _get_token_from_file():
        logger.warning(f"Active token '{token_name}' has been deleted.")
        Path(constants.MEGA_TOKEN_PATH).unlink(missing_ok=True)
...
def _set_active_token(
    ...
    token = _get_token_by_name(token_name)
    if not token:
        raise ValueError(f"Token {token_name} not found in {constants.MEGA_STORED_TOKENS_PATH}")
    if add_to_git_credential:
    ...
    # Write token to the active token file.
    _write_secret(Path(constants.MEGA_TOKEN_PATH), token)
    logger.debug("The active token was saved.")
...
def _is_git_credential_helper_configured() -> bool:
    ...
    # See https://github.com/huggingface/megatensors._hub/issues/1043#issuecomment-1247010710
    if is_google_colab():
    ...
    print(
        ANSI.red(
            ...
            " machine.\nYou might have to re-authenticate when pushing to MEGA Hub."
...
def _set_store_as_git_credential_helper_globally() -> None:
    ...
    Related:
    - https://github.com/huggingface/megatensors._hub/issues/1043
    - https://github.com/huggingface/megatensors._hub/issues/1051
