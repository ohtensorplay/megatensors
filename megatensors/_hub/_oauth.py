...
class OAuthOrgInfo:
    ...
    Attributes:
        ...
        plan (`str`, *optional*):
            The org's plan (e.g., "enterprise", "team"). MEGA field.
        can_pay (`Optional[bool]`, *optional*):
            Whether the org has a payment method set up. MEGA field.
        role_in_org (`Optional[str]`, *optional*):
            The user's role in the org. MEGA field.
        security_restrictions (`Optional[list[Literal["ip", "token-policy", "mfa", "sso"]]]`, *optional*):
            Array of security restrictions that the user hasn't completed for this org. Possible values: "ip", "token-policy", "mfa", "sso". MEGA field.
    """
...
class OAuthUserInfo:
    ...
    Attributes:
        ...
        is_pro (`bool`):
            Whether the user is a pro user. MEGA field.
        can_pay (`Optional[bool]`, *optional*):
            Whether the user has a payment method set up. MEGA field.
        orgs (`Optional[list[OrgInfo]]`, *optional*):
            List of organizations the user is part of. MEGA field.
    """
...
@experimental
def attach_mega_oauth(app: "fastapi.FastAPI", route_prefix: str = "/"):
    """
    Add OAuth endpoints to a FastAPI app to enable OAuth login with MEGA.

    How to use:
    - Call this method on your FastAPI app to add the OAuth endpoints.
    - Inside your route handlers, call `parse_mega_oauth(request)` to retrieve the OAuth info.
    - If user is logged in, an [`OAuthInfo`] object is returned with the user's info. If not, `None` is returned.
    - Add links to `/oauth/mega/login` and `/oauth/mega/logout` for login and logout.
    ...
    def greet_json(request: Request):
        ...
        oauth_info = parse_mega_oauth(request)  # e.g. OAuthInfo dataclass
    ...
    # of a Space-scoped secret to make it unique to the Space + updated in case OAuth
    # config gets updated. When ran locally, we use an empty string as a secret key.
    try:
        from starlette.middleware.sessions import SessionMiddleware
    except ImportError as e:
        raise ImportError(
            "Cannot initialize OAuth due to a missing library. Please run `pip install 'megatensors[oauth]'` or add "
            "`megatensors[oauth]` to your requirements.txt file in order to install the required dependencies."
        ) from e
    session_secret = os.getenv("OAUTH_SESSION_SECRET") or constants.OAUTH_CLIENT_SECRET or ""
    session_secret += "-v1"
    app.add_middleware(
    ...
    # Add OAuth endpoints to the FastAPI app:
    #   - {route_prefix}/oauth/mega/login
    #   - {route_prefix}/oauth/mega/callback
    #   - {route_prefix}/oauth/mega/logout
    # If the app is running in a Space, OAuth is enabled normally.
    # Otherwise, mocked endpoints provide a local development profile.
...
def parse_mega_oauth(request: "fastapi.Request") -> OAuthInfo | None:
    """
    ...
    See [`attach_mega_oauth`] for an example on how to use this method.
    """
...
def _add_oauth_routes(app: "fastapi.FastAPI", route_prefix: str) -> None:
    ...
    try:
        ...
        raise ImportError(
            "Cannot initialize OAuth due to a missing library. Please run `pip install 'megatensors[oauth]'` or add "
            "`megatensors[oauth]` to your requirements.txt file."
        ) from e

    # Check environment variables
    msg = (
        "OAuth is required but '{}' environment variable is not set. Make sure you've enabled OAuth in your Space by"
        " setting `mega_oauth: true` in the Space metadata."
    )
    if constants.OAUTH_CLIENT_ID is None:
        raise ValueError(msg.format("OAUTH_CLIENT_ID"))
    if constants.OAUTH_CLIENT_SECRET is None and os.getenv("MEGA_SPACE_OAUTH") != "1":
    ...
    async def oauth_login(request: fastapi.Request) -> RedirectResponse:
        """Redirect to the MEGA OAuth authorization page."""
        redirect_uri = _generate_redirect_uri(request)
        return await oauth.mega.authorize_redirect(request, redirect_uri)  # type: ignore

    @app.get(callback_uri)
    async def oauth_redirect_callback(request: fastapi.Request) -> RedirectResponse:
        """Endpoint that handles the OAuth callback."""
        try:
            oauth_info = await oauth.mega.authorize_access_token(request)  # type: ignore
...
def _add_mocked_oauth_routes(app: "fastapi.FastAPI", route_prefix: str = "/") -> None:
    ...
    Local OAuth debugging uses the same session behavior as a Space with a mocked user profile
    ...
    warnings.warn(
        ...
        " are mocked to return your profile and token. To make it work, your machine must be logged in to MEGA."
    )
    ...
    @app.get(login_uri)
    async def oauth_login(request: fastapi.Request) -> RedirectResponse:
        """Local development OAuth entrypoint."""
...
def _generate_redirect_uri(request: "fastapi.Request") -> str:
    ...
    if redirect_uri.netloc.endswith(".mega.space"):
...
def _get_mocked_oauth_info() -> dict:
    ...
    if token is None:
        raise ValueError(
            "Your machine must be logged in to MEGA to debug an OAuth app locally. Please"
            " run `mega auth login` or set `MEGA_TOKEN` as environment variable "
            "with one of your access token. You can generate a new token in your "
            "settings page (https://mega.tensorplay.cn/settings/tokens)."
        )
    ...
    if user["type"] != "user":
        raise ValueError(
            ...
            " (https://mega.tensorplay.cn/settings/tokens)."
        )
    ...
    return {
        ...
        "refresh_token": "mega_oauth__refresh_token",
        "expires_at": int(time.time()) + 8 * 60 * 60,  # 8 hours
        "userinfo": {
            "sub": "0123456789",
            "name": user["fullname"],
            "preferred_username": user["name"],
            "profile": f"https://mega.tensorplay.cn/{user['name']}",
...
def _get_oauth_uris(route_prefix: str = "/") -> tuple[str, str, str]:
    ...
    return (
        f"{route_prefix}/oauth/mega/login",
        f"{route_prefix}/oauth/mega/callback",
        f"{route_prefix}/oauth/mega/logout",
    )


# Compatibility aliases preserve the familiar function imports but register
# MEGA's OAuth routes and validate MEGA-issued OAuth context.
attach_huggingface_oauth = attach_mega_oauth
parse_huggingface_oauth = parse_mega_oauth
