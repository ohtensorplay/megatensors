...
for a short-lived MEGA token — no long-lived ``MEGA_TOKEN`` secret to store.
...
The token exchange is served by the configured MEGA endpoint.
"""
...
def get_oidc_token(*, provider: Provider | str | None = None, audience: str | None = None) -> str:
    ...
    Args:
        ...
        audience (`str`, *optional*):
            ...
            that validates it (respects `MEGA_ENDPOINT`/staging).
...
def exchange_oidc_token(*, subject_token: str, resource: str, endpoint: str | None = None) -> dict:
    """Exchange a CI OIDC id token for a short-lived MEGA token (RFC 8693).
    ...
    Returns:
        ...
        `{"access_token": "mega_jwt_…", "token_type": "bearer", "expires_in": 3600, ...}`.
    """
...
def oidc_login(
    *,
    ...
    """Mint a CI OIDC id token and exchange it for a MEGA token.
