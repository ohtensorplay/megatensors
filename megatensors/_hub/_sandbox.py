...
from .errors import MegaHubHTTPError, SandboxCommandError, SandboxError
from .mega_api import MegaApi, JobInfo
...
SANDBOX_LABEL = "mega-sandbox"
# Sandbox mode: "dedicated" or "pool" to ease filtering
MODE_LABEL = "mega-sandbox-mode"
...
NONCE_LABEL = "mega-sandbox-nonce"
...
def _derive_sandbox_token(mega_token: str, nonce: str) -> str:
    """Derive the per-sandbox auth token from the user's MEGA token and the sandbox nonce.

    Stateless: any machine holding the same MEGA token can recompute it from the
    nonce stored in the job's labels, so `Sandbox.connect(job_id)` needs no local state.
    The MEGA token itself is never sent to the sandbox.
    """
    return hmac.new(mega_token.encode(), f"mega-sandbox:{nonce}".encode(), hashlib.sha256).hexdigest()
...
class Sandbox:
    """An isolated cloud machine running on MEGA Jobs.
    ...
    ```python
    >>> from megatensors._hub import Sandbox
        ...
        *,
        ...
        Args:
            image (`str`, *optional*, defaults to `"python:3.12"`):
                Any Docker image with `/bin/sh` (Docker Hub or `mega.tensorplay.cn/spaces/...`).
            flavor (`str`, *optional*, defaults to `"cpu-basic"`):
                Hardware flavor, e.g. `"cpu-basic"`, `"a10g-small"`. See `mega jobs hardware`.
            ...
            volumes (`List[Volume]`, *optional*):
                MEGA repos/buckets to mount, see [`Volume`].
            namespace (`str`, *optional*):
                User or org namespace to run under (defaults to current user).
            forward_mega_token (`bool`, *optional*, defaults to `False`):
                If True, your MEGA token is injected as `MEGA_TOKEN` (opt-in).
        ...
        mega_token = _effective_token(api)
        nonce = token_hex(16)
        sandbox_token = _derive_sandbox_token(mega_token, nonce)
        ...
        command, job_env, job_secrets, job_volumes = _bootstrap_job_spec(
            ...
            forward_mega_token=forward_mega_token,
            sandbox_token=sandbox_token,
        )
    ...
    def proxy_url_for(self, port: int | str, path: str = "/", *, scheme: str = "https://") -> str:
        ...
        Returns:
            `str`: a URL like `https://<job_id>--49983.mega.jobs/v1/.../proxy/8000/ws` (or
            `wss://...` with `scheme="wss://"`).
    ...
    def proxy_headers(self) -> dict[str, str]:
        """Auth headers to send with [`proxy_url_for`] requests (MEGA token + sandbox token)."""
        return {
...
class SandboxPool:
    ...
    ```python
    >>> from megatensors._hub import SandboxPool
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            name (`str`, *optional*):
                Pool name, used as the `mega-sandbox-pool` job label so the pool is
                discoverable (e.g. `mega sandbox pool ls`, `connect()`). `create()` reuses
    ...
    def create(
        ...
        *,
        ...
        Args:
            ...
            forward_mega_token (`bool`, *optional*, defaults to `False`):
                If True, inject your MEGA token as `MEGA_TOKEN` in the sandbox
        ...
        if forward_mega_token:
            sandbox_env["MEGA_TOKEN"] = _effective_token(self._api)
    ...
    def close(self) -> None:
        ...
        with `mega sandbox pool delete <id>`.
        """
        with self._lock:
            hosts = self._hosts
            self._hosts = []
    ...
    def _ensure_warmed_up(self) -> None:
        ...
        # Persist the warmed hosts so the next process (e.g. `mega sandbox create --pool` after a
        # bare `mega sandbox pool create`) hits the cache fast path instead of re-discovering.
        if self._hosts:
            self._save_cache()
    ...
    def _discover_hosts(self) -> None:
        ...
        for job in matches:
            ...
            except (SandboxError, httpx.HTTPError, MegaHubHTTPError) as e:
    ...
    def _boot_host(self) -> _SandboxServer:
        ...
        mega_token = _effective_token(self._api)
        nonce = token_hex(16)
        sandbox_token = _derive_sandbox_token(mega_token, nonce)
        command, job_env, job_secrets, job_volumes = _bootstrap_job_spec(
            self._api,
        ...
        with self._lock:
            ...
            for ch in cache.hosts:
                ...
                server = _SandboxServer(
                    ...
                    nonce=ch.nonce,
                    sandbox_token=_derive_sandbox_token(mega_token, ch.nonce),
                    api=self._api,
                    max_connections=min(self.sandboxes_per_host + 8, 256),
...
def _effective_token(api: MegaApi) -> str:
    token = api.token if isinstance(api.token, str) else get_token()
    if not token:
        raise SandboxError("A MEGA token is required to use sandboxes. Run `mega auth login` first.")
    return token
...
def _host_env(api: MegaApi, job: JobInfo, *, namespace: str | None) -> dict[str, Any]:
...
def _find_pool_host_job(api: MegaApi, pool_id: str, *, namespace: str | None = None) -> JobInfo:
...
def _connect_host(api: MegaApi, host_job_id: str, *, namespace: str | None = None) -> _SandboxServer:
...
def _split_sandbox_id(sandbox_id: str, namespace: str | None) -> tuple[str, str | None]:
    """Accept `namespace/sandbox_id` ids (as shown in the Hub UI), like `mega jobs` does."""
...
def _tail_job_logs(api: MegaApi, job_id: str, *, namespace: str | None = None, limit: int = 20) -> str:
    try:
