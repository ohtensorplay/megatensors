...
from .mega_api import DatasetInfo, MegaApi, KernelInfo, ModelInfo, RepoFile, SpaceInfo
from .utils import (
...
from .utils.tqdm import tqdm as mega_tqdm
...
def snapshot_download(
    ...
    *,
    ...
    Args:
        ...
        per_file_progress (`bool`, *optional*, defaults to `False`):
            If `True`, show the individual file download progress bars instead of aggregating all file bytes
            into the snapshot-level transfer and reconstruction bars. Intended for human CLI output.
    ...
    tree_cache_folder = (
        tree_cache_folder_for_local_dir(local_dir)
        if local_dir is not None
        else storage_folder
    )
    ...
    if not local_files_only:
        # try/except logic to handle different errors => taken from `mega_hub_download`
        try:
    ...
    # - internet connection is down
    # - internet connection is deactivated (local_files_only=True or MEGA_HUB_OFFLINE=True)
    ...
    if repo_info is None:
        ...
        if local_files_only:
            ...
            raise LocalEntryNotFoundError(
                ...
                "'MEGA_HUB_OFFLINE=0' as environment variable."
            ) from api_call_error
        elif isinstance(api_call_error, (RepositoryNotFoundError, GatedRepoError)) or (
            isinstance(api_call_error, MegaHubHTTPError)
    ...
    # User can use its own tqdm class or the default one from `megatensors._hub.utils`
    tqdm_class = tqdm_class or mega_tqdm
    ...
    if per_file_progress:
        inner_tqdm_class = tqdm_class
        transfer_progress = None
        reconstruct_progress = None
    else:
        ...
        transfer_progress = _create_progress_bar(
            ...
            name="megatensors._hub.snapshot_download.transfer",
        ...
        reconstruct_progress = _create_progress_bar(
            cls=tqdm_class,
            log_level=logger.getEffectiveLevel(),
            name="megatensors._hub.snapshot_download",
        ...
        inner_tqdm_class = _AggregatedTqdm  # type: ignore[assignment]

    # Pass the commit_hash as revision to mega_hub_download to skip network call if:
    # - file is cached
    # - or xet file with metadata cached in /tree cache
    def _inner_mega_hub_download(repo_file: str) -> None:
        results.append(
            mega_hub_download(  # type: ignore
                repo_id,
                filename=repo_file,
                repo_type=repo_type,
                revision=commit_hash,
                ...
                token=token,
                headers=headers,
                tqdm_class=inner_tqdm_class,  # type: ignore
                dry_run=dry_run,
            )
        )

    thread_map(
        _inner_mega_hub_download,
        filtered_repo_files,
        desc=tqdm_desc,
    ...
    if transfer_progress is not None and reconstruct_progress is not None:
...
def get_cached_repo_tree(
    ...
    *,
    ...
    Args:
        ...
        cache_dir (`str`, `Path`, *optional*):
            Path to the folder where cached files are stored. Defaults to the value of `MEGA_HUB_CACHE`.
        local_dir (`str` or `Path`, *optional*):
            If provided, read the tree listing cached by a `local_dir` download (from
            `local_dir/.cache/mega/`) instead of the main cache. Branch/tag revisions are still resolved
    ...
    Example:
        ```py
        >>> from megatensors._hub import get_cached_repo_tree
    ...
    # For `local_dir` downloads the tree listing lives under `local_dir/.cache/mega/`; otherwise it lives
...
def _local_file_exists(base_dir: str, path: str) -> bool:
    ...
    if (
        os.name == "nt"
        and len(os.path.abspath(full_path)) > 255
