...
from megatensors._hub.utils import MegaMount, MegaUri, parse_datetime
...
class SpaceHardware(str, Enum):
    """
    Hugging Face-compatible Space hardware identifiers.

    This static compatibility type is not an availability guarantee for a MEGA
    deployment. Use ``list_spaces_hardware()`` to discover the live catalogue
    before asking MEGA to run a Space on a particular flavor.
...
class Volume:
    ...
    def to_uri(self) -> str:
        ...
        return MegaMount(
            source=MegaUri(type=self.type, id=self.source, revision=self.revision, path_in_repo=self.path or ""),
...
class SpaceRuntime:
    ...
    Args:
        ...
        sleep_time (`int` or `None`):
            ...
            hours on a free 'cpu-basic' hardware. For more details, see https://mega.tensorplay.cn/docs/hub/spaces-gpus#sleep-time.
...
class SpaceSearchResult:
    ...
    Returned by [`MegaApi.search_spaces`].
...
class SpaceTemplate:
    ...
    Returned by [`MegaApi.list_space_templates`]. The `repo_id` can be passed as `space_template`
