...
class MegaSummaryWriter(_RuntimeSummaryWriter):
    """
    ...
    > [!WARNING]
    > `MegaSummaryWriter` is experimental. Its API is subject to change in the future without prior notice.
    ...
    Args:
        ...
        repo_allow_patterns (`list[str]` or `str`, *optional*):
            ...
            [upload guide](https://mega.tensorplay.cn/docs/megatensors._hub/guides/upload#upload-a-folder) for more details.
        repo_ignore_patterns (`list[str]` or `str`, *optional*):
            A list of patterns to exclude in the upload. Check out the
            [upload guide](https://mega.tensorplay.cn/docs/megatensors._hub/guides/upload#upload-a-folder) for more details.
        token (`str`, *optional*):
            Authentication token. Will default to the stored token. See https://mega.tensorplay.cn/settings/token for more
            details
        kwargs:
    ...
    + from megatensors._hub import MegaSummaryWriter

    import numpy as np

    - writer = SummaryWriter()
    + writer = MegaSummaryWriter(repo_id="username/my-trained-model")
    ...
    >>> with MegaSummaryWriter(repo_id="test_hf_logger", commit_every=15) as logger:
    ...
    @experimental
    def __new__(cls, *args, **kwargs) -> "MegaSummaryWriter":
        if not is_summary_writer_available:
            raise ImportError(
                "You must have `tensorboard` installed to use `MegaSummaryWriter`. Please run `pip install --upgrade"
                " tensorboardX` first."
            )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Add `mega-summary-writer` tag to the model card metadata
        try:
            card = ModelCard.load(repo_id_or_path=self.repo_id, repo_type=self.repo_type)
        except EntryNotFoundError:
            card = ModelCard("")
        tags = card.data.get("tags", [])
        if "mega-summary-writer" not in tags:
            tags.append("mega-summary-writer")
            card.data["tags"] = tags
...
# Compatible import spelling; the writer continues to publish to MEGA Hub.
HFSummaryWriter = MegaSummaryWriter
