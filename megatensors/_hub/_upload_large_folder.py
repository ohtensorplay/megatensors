...
# Repository limits (from https://mega.tensorplay.cn/docs/hub/repositories-recommendations)
...
def _validate_upload_limits(paths_list: list[LocalUploadFilePaths]) -> None:
    ...
    if len(paths_list) > MAX_FILES_PER_REPO:
        logger.warning(
            ...
            f"  - See: https://mega.tensorplay.cn/docs/hub/repositories-recommendations"
        )
...
def upload_large_folder_internal(
    ...
    *,
    ...
    See [`MegaApi.upload_large_folder`] for the full documentation.
    """
    # 1. Check args and setup
    if repo_type is None:
        raise ValueError(
            "For large uploads, `repo_type` is explicitly required. Please set it to `model`, `dataset`, `space` or `mcp`."
    ...
    try:
        ...
        if commit_count > 500:
            logger.warning(
                ...
                f"  from megatensors._hub import super_squash_history\n"
                ...
                f"https://mega.tensorplay.cn/docs/megatensors._hub/main/en/package_reference/mega_api#megatensors._hub.MegaApi.super_squash_history\n"
                f"{'=' * 80}\n"
            )
...
def _get_upload_mode(items: list[JOB_ITEM_T], api: "MegaApi", repo_id: str, repo_type: str, revision: str) -> None:
    ...
    _fetch_upload_modes(
        ...
        repo_type=repo_type,
        repo_id=repo_id,
        headers=api._build_mega_headers(),
...
def _preupload_lfs(items: list[JOB_ITEM_T], api: "MegaApi", repo_id: str, repo_type: str, revision: str) -> None:
...
def _commit(items: list[JOB_ITEM_T], api: "MegaApi", repo_id: str, repo_type: str, revision: str) -> None:
