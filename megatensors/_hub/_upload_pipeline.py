...
if TYPE_CHECKING:
    from .mega_api import CommitInfo, MegaApi
...
class _LiveDisplay:
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(target=self._render_loop, name="mega-upload-display", daemon=True)
        self._thread.start()
...
class _UploadPipeline:
    def __init__(
        ...
        *,
        ...
        self.token = token
        self.headers = api._build_mega_headers(token=token)
    ...
    def run(self) -> "CommitInfo":
        ...
        committer = threading.Thread(target=self._committer_loop, name="mega-upload-committer", daemon=True)
        committer.start()
        self.display.start()
        try:
    ...
    def _coordinator_loop(self) -> None:
        ...
        for start in range(0, len(self.add_operations), PREUPLOAD_BATCH_SIZE):
            ...
            try:
                ...
                from .mega_api import _CREATE_COMMIT_NO_REPO_ERROR_MESSAGE
