...
"""Shell completion for the ``mega`` CLI, built on Click's native completion.

Click generates completion scripts activated by the ``_MEGA_COMPLETE`` env var (this
works out of the box because ``mega`` is a Click command). This module exposes the two
...
_COMPLETE_VAR = "_MEGA_COMPLETE"

# Shells with a shared rc file: the line that activates completion (appended once).
_RC_ACTIVATION: dict[str, tuple[Path, str]] = {
    "bash": (Path.home() / ".bashrc", f'eval "$({_COMPLETE_VAR}=bash_source mega)"'),
    "zsh": (Path.home() / ".zshrc", f'eval "$({_COMPLETE_VAR}=zsh_source mega)"'),
}
# Fish auto-loads per-command files from its completions directory. The file is dedicated
# to `mega`, so the full script is written there directly (overwriting any stale version)
# instead of re-generating it via `mega` on every shell startup.
_FISH_COMPLETION_PATH = Path.home() / ".config" / "fish" / "completions" / "mega.fish"
...
def _completion_script(shell: str) -> str:
    # Imported lazily to avoid a circular import (mega.py imports the options below).
    ...
    return completion_cls(app, {}, "mega", _COMPLETE_VAR).source()
