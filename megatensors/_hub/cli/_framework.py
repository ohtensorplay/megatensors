...
"""MEGA's typed command declaration layer over Click 8.x.

The module turns annotated command signatures
(``Annotated[T, Option(...)]`` / ``Argument(...)``) into Click parameters and
provides the help behavior used throughout the MEGA command tree. It depends
only on Click's public API. Click handles exceptions, exits, and completion;
this module owns MEGA's annotation conversion and command construction rules.
"""
...
def _enum_convertor(enum_type: type[enum.Enum]) -> Callable[[Any], Any]:
    """Map a parsed choice string back to its ``Enum`` member."""
...
def _make_convertor(
    ...
    An explicit ``click_type`` owns its own conversion, so no enum convertor is
    added on top of it.
    """
    inner = None
...
def _wrap_param_callback(
    ...
    """Adapt a value parser ``fn(value) -> parsed`` to a Click callback."""
...
# ---------------------------------------------------------------------------
# 4. Click parameter subclasses — arguments show up in ``--help`` (plain
#    ``click.Argument`` renders none) with MEGA's default/metavar formatting.
# ---------------------------------------------------------------------------
...
def _default_string(
    param: "MegaArgument | MegaOption", ctx: click.Context, show_default_is_str: bool, default_value: Any
) -> str:
    ...
    if inspect.isfunction(default_value):
        return "(dynamic)"
    if isinstance(param, MegaOption) and param.is_bool_flag and param.secondary_opts:
    ...
    if isinstance(param, MegaOption) and param.is_bool_flag and not param.secondary_opts and not default_value:
        return ""
...
def _build_help_extra(param: "MegaArgument | MegaOption", ctx: click.Context, base_help: str) -> str:
    ...
    # Numeric range hints are shown for options only; arguments omit them.
    if isinstance(param, MegaOption) and isinstance(param.type, (click.IntRange, click.FloatRange)):
...
class MegaArgument(click.Argument):
...
class MegaOption(click.Option):
    """Option with MEGA's default and metavar rendering."""
...
class MegaCommand(click.Command):
...
def build_command(
    ...
    *,
    ...
    The resulting command invokes the original annotated function directly.
    """
...
class MegaGroup(click.Group):
    """Command group with the decorator API the CLI relies on (``command``/``callback``/``add_group``).

    Subclasses (see ``MegaCliGroup``) layer on styling, aliases, topics and error
    ...
    command_class: type[click.Command] = MegaCommand
    ...
    def group_callback(
        ...
        def decorator(func: Callable[..., Any]) -> Callable[..., Any]:
            ...
            if invoke_without_command:
                # The subcommand is optional when the group runs without one.
