...
"""Output framework for the `mega` CLI."""
...
from megatensors._hub.errors import ConfirmationError
...
class Output:
    ...
    def reset_banner(self) -> None:
        """Allow a fresh CLI invocation in the same Python process to print a banner."""
        self._banner_printed = False
    ...
    def banner(self) -> None:
        """Print the MEGA CLI banner once for human-facing commands."""
        if self.mode != OutputFormat.human or self._banner_printed:
            return
        _print_flush(ANSI.green(_MEGA_ASCII), file=sys.stderr)
        _print_flush(file=sys.stderr)
    ...
    def table(
        ...
        *,
        ...
        match self.mode:
            case (
                OutputFormat.human
            ):  # padded table, adaptive truncation, SCREAMING_SNAKE headers
