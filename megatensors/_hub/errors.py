...
class CacheNotFound(Exception):
    """Exception thrown when the MEGA cache is not found."""
...
class CorruptedCacheException(Exception):
    """Exception for any unexpected structure in the MEGA cache-system."""
...
class OIDCError(Exception):
    ...
    Typically because `MEGA_OIDC_RESOURCE` is set but no id token is available: not running in a
    supported CI provider and `MEGA_OIDC_ID_TOKEN` is unset.

    See the MEGA trusted-publisher documentation.
    """
...
class LoginError(Exception):
    """User-safe error raised while starting, validating, or saving a login."""
...
class DeviceCodeError(LoginError):
...
class OfflineModeIsEnabled(ConnectionError):
    """Raised when a request is made but `MEGA_HUB_OFFLINE=1` is set as environment variable."""


class MegaHubHTTPError(HTTPError, OSError):
    """
    HTTPError to inherit from for any custom HTTP Error raised in MEGA Hub.

    Any HTTPError is converted at least into a `MegaHubHTTPError`. If some information is
    ...
    ```py
        import httpx
        from megatensors._hub.utils import get_session, mega_raise_for_status, MegaHubHTTPError
    ...
    def append_to_message(self, additional_message: str) -> None:
        """Append additional information to the `MegaHubHTTPError` initial message."""
        self.args = (self.args[0] + additional_message,) + self.args[1:]

    @classmethod
    def _reconstruct_mega_hub_http_error(
    ...
    def __reduce_ex__(self, protocol):
        ...
        return (self.__class__._reconstruct_mega_hub_http_error, (str(self), self.response, self.server_message))
...
class MegaValidationError(ValueError):
    """Generic exception thrown by `megatensors._hub` validators.

    Inherits from [`ValueError`](https://docs.python.org/3/library/exceptions.html#ValueError).
    """


class MegaUriError(ValueError):
    """Raised when a `mega://...` URI is malformed.
    ...
    def __init__(self, uri: str, msg: str):
        self.uri = uri
        self.msg = msg
        full_msg = f"Invalid MEGA URI '{uri}'. {msg}" if uri else f"Invalid MEGA URI. {msg}"
...
class BucketNotFoundError(MegaHubHTTPError):
    """
    ...
    (...)
    megatensors._hub.errors.BucketNotFoundError: 404 Client Error. (Request ID: XXX)

    Bucket Not Found for url: https://mega.tensorplay.cn/api/buckets/namespace/name.
...
class JobNotFoundError(MegaHubHTTPError):
    """
...
class RepositoryNotFoundError(MegaHubHTTPError):
    """
    Raised when trying to access a MEGA Hub URL with an invalid repository name, or
    ...
    ```py
    >>> from megatensors._hub import model_info
    >>> model_info("<non_existent_repository>")
    (...)
    megatensors._hub.errors.RepositoryNotFoundError: 401 Client Error. (Request ID: PvMw_VjBMjVdMz53WKIzP)

    Repository Not Found for url: https://mega.tensorplay.cn/api/models/%3Cnon_existent_repository%3E.
...
class GatedRepoError(RepositoryNotFoundError):
    ...
    (...)
    megatensors._hub.errors.GatedRepoError: 403 Client Error. (Request ID: ViT1Bf7O_026LGSQuVqfa)

    Cannot access gated repo for url https://mega.tensorplay.cn/api/models/ardent-figment/gated-model.
    Access to model ardent-figment/gated-model is restricted and you are not in the authorized list.
    Visit https://mega.tensorplay.cn/ardent-figment/gated-model to ask for access.
    ```
    """


class DisabledRepoError(MegaHubHTTPError):
    """
    ...
    ```py
    >>> from megatensors._hub import dataset_info
    >>> dataset_info("laion/laion-art")
    (...)
    megatensors._hub.errors.DisabledRepoError: 403 Client Error. (Request ID: Root=1-659fc3fa-3031673e0f92c71a2260dbe2;bc6f4dfb-b30a-4862-af0a-5cfe827610d8)

    Cannot access repository for url https://mega.tensorplay.cn/api/datasets/laion/laion-art.
...
class RevisionNotFoundError(MegaHubHTTPError):
    """
    Raised when trying to access a MEGA Hub URL with a valid repository but an invalid
    revision.
    ...
    ```py
    >>> from megatensors._hub import mega_hub_download
    >>> mega_hub_download('bert-base-cased', 'config.json', revision='<non-existent-revision>')
    (...)
    megatensors._hub.errors.RevisionNotFoundError: 404 Client Error. (Request ID: Mwhe_c3Kt650GcdKEFomX)

    Revision Not Found for url: https://mega.tensorplay.cn/bert-base-cased/resolve/%3Cnon-existent-revision%3E/config.json.
    ```
    """
...
class EntryNotFoundError(Exception):
    ...
    ```py
    >>> from megatensors._hub import mega_hub_download
    >>> mega_hub_download('bert-base-cased', '<non-existent-file>')
    (...)
    megatensors._hub.errors.RemoteEntryNotFoundError (...)
    >>> mega_hub_download('bert-base-cased', '<non-existent-file>', local_files_only=True)
    (...)
    megatensors._hub.utils.errors.LocalEntryNotFoundError (...)
    ```
    """


class RemoteEntryNotFoundError(MegaHubHTTPError, EntryNotFoundError):
    """
    Raised when trying to access a MEGA Hub URL with a valid repository and revision
    ...
    >>> mega_hub_download('bert-base-cased', '<non-existent-file>')
    (...)
    megatensors._hub.errors.EntryNotFoundError: 404 Client Error. (Request ID: 53pNl6M0MxsnG5Sw8JA6x)

    Entry Not Found for url: https://mega.tensorplay.cn/bert-base-cased/resolve/main/%3Cnon-existent-file%3E.
    ```
    """
...
class LocalEntryNotFoundError(FileNotFoundError, EntryNotFoundError):
    ...
    ```py
    >>> from megatensors._hub import mega_hub_download
    >>> mega_hub_download('bert-base-cased', '<non-cached-file>',  local_files_only=True)
    (...)
    megatensors._hub.errors.LocalEntryNotFoundError: Cannot find the requested files in the disk cache and outgoing traffic has been disabled. To enable MEGA Hub look-ups and downloads online, set 'local_files_only' to False.
    ```
    """
...
# REQUEST ERROR
class BadRequestError(MegaHubHTTPError, ValueError):
    """
    Raised by `mega_raise_for_status` when the server returns a HTTP 400 error.

    Example:

    ```py
    >>> resp = httpx.post("MEGA Hub/api/check", ...)
    >>> mega_raise_for_status(resp, endpoint_name="check")
    megatensors._hub.errors.BadRequestError: Bad request for check endpoint: {details} (Request ID: XXX)
    ```
    """
...
class SandboxError(Exception):
    """Base exception for sandbox operations (see `megatensors._hub.Sandbox`).
