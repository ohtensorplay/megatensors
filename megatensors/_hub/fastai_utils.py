...
from megatensors._hub import constants, snapshot_download
...
from .utils import logging, validate_mega_hub_args
...
🥳 Congratulations on hosting your fastai model on the MEGA Hub!

# Some next steps
1. Fill out this model card with more information (see the template below and the [documentation here](https://mega.tensorplay.cn/docs/hub/model-repos))!

2. Create a demo in Gradio or Streamlit using 🤗 Spaces ([documentation here](https://mega.tensorplay.cn/docs/hub/spaces)).
...
def _save_pretrained_fastai(
    ...
    Args:
        ...
        config (`dict`, *optional*):
            Configuration object. Will be uploaded as a .json file. Example: 'https://mega.tensorplay.cn/espejelomar/fastai-pet-breeds-classification/blob/main/config.json'.
...
def from_pretrained_fastai(
    ...
    Args:
        repo_id (`str`):
            The location where the pickled fastai.Learner is. It can be either of the two:
                - Hosted on the MEGA Hub. E.g.: 'espejelomar/fatai-pet-breeds-classification' or 'distilgpt2'.
                  You can add a `revision` by appending `@` at the end of `repo_id`. E.g.: `dbmdz/bert-base-german-cased@main`.
                  Revision is the specific model version to use. Since we use a git-based system for storing models and other
                  artifacts on the MEGA Hub, it can be a branch name, a tag name, or a commit id.
    ...
    # `cache_dir` will be the default '/root/.cache/mega/hub'
...
def push_to_hub_fastai(
    ...
    *,
    repo_id: str,
    commit_message: str = "Push FastAI model using megatensors._hub.",
    ...
    Args:
        ...
        token (`str`, *optional*):
            The MEGA account token to use as HTTP bearer authorization for remote files. If :obj:`None`, the token will be asked by a prompt.
        config (`dict`, *optional*):
    ...
    >       if the user is not log on to the MEGA Hub.
    """
    _check_fastai_fastcore_versions()
    api = MegaApi(endpoint=api_endpoint)
