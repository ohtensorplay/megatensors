...
def are_symlinks_supported(cache_dir: str | Path | None = None) -> bool:
    ...
    on the precise cache folder. By default, the default MEGA cache directory is checked.
    ...
    """
    # Defaults to MEGA cache
    if cache_dir is None:
        cache_dir = constants.MEGA_HUB_CACHE
    cache_dir = str(Path(cache_dir).expanduser().resolve())  # make it unique

    # If symlinks are explicitly disabled by the user, always return False
    if constants.MEGA_HUB_DISABLE_SYMLINKS:
        return False
    ...
    if cache_dir not in _are_symlinks_supported_in_dir:
        ...
        with SoftTemporaryDirectory(dir=cache_dir) as tmpdir:
            ...
            try:
                ...
                if not constants.MEGA_HUB_DISABLE_SYMLINKS_WARNING:
                    message = (
                        "`megatensors._hub` cache-system uses symlinks by default to"
                        ...
                        " `MEGA_HUB_DISABLE_SYMLINKS_WARNING` environment variable. For"
                        " more details, see"
                        " https://mega.tensorplay.cn/docs/megatensors._hub/how-to-cache#limitations."
                    )
                    if os.name == "nt":
                        message += (
...
class MegaFileMetadata:
    ...
    Returned by [`get_mega_file_metadata`] based on a URL.
...
# Preserve the public Hugging Face spelling for source-compatible migrations.
# The object and its metadata still describe files served by MEGA Hub.
HfFileMetadata = MegaFileMetadata
...
class DryRunFileInfo:
    ...
    Returned by [`mega_hub_download`] when `dry_run=True`.
    ...
    Args:
        ...
        will_download (`bool`):
            Whether the file will be downloaded if `mega_hub_download` is called with `dry_run=False`.
...
def mega_hub_url(
    ...
    *,
    ...
    The resolved address is a MEGA Hub URL. The service may redirect it to a
    signed object-storage URL for large objects.
    ...
    ```python
    >>> from megatensors._hub import mega_hub_url

    >>> mega_hub_url(
    ...     repo_id="julien-c/EsperBERTo-small", filename="pytorch_model.bin"
    ... )
    'https://mega.tensorplay.cn/julien-c/EsperBERTo-small/resolve/main/pytorch_model.bin'
    ```
    ...
    >     revisioned repository model, which means that we store
    ...
    # The MEGA Worker identifies every repository as ``owner/name`` and keeps
    # the repository type as metadata. Its resolve route therefore must not
    # receive the historic ``datasets/`` or ``spaces/`` path prefix.
...
def http_get(
    ...
    *,
    ...
    """
    Download a remote file. Do not gobble up errors, and will return errors tailored to the MEGA Hub.
    ...
    if resume_size > 0:
        ...
        raise ValueError(
            ...
            "Install the optional Xet accelerator for Xet-backed downloads."
        )
    ...
    with http_stream_backoff(
        ...
        if resume_size > 0 and response.status_code == 200:
            ...
            if _tqdm_bar is not None:
                ...
                if callable(
                    update_transfer := getattr(_tqdm_bar, "update_transfer", None)
                ):
        ...
        if total is None:
            ...
            # `expected_size` (known from the MEGA metadata HEAD request) so the progress bar, and any
        ...
        progress_cm = _get_progress_bar_context(
            desc=f"Downloading [{displayed_filename}]",
            log_level=logger.getEffectiveLevel(),
            total=total,
            initial=resume_size,
            name="megatensors._hub.http_get",
            ...
            try:
                for chunk in response.iter_bytes(
                    ...
                    if chunk:  # filter out keep-alive new chunks
                        ...
                        if callable(
                            update_transfer := getattr(
                                progress, "update_transfer", None
                            )
                        ):
...
def xet_get(
    ...
    **How it works:**
    ...
    try:
        ...
        raise ValueError(
            "Xet-backed downloads require the optional Xet accelerator package."
        )
    ...
    with XetDownloadProgressReporter(
        reconstruction_desc=f"Reconstructing [{displayed_filename}]",
        transfer_desc=f"Downloading [{displayed_filename}]",
        total=expected_size,
        log_level=logger.getEffectiveLevel(),
        name="megatensors._hub.xet_get",
        tqdm_class=tqdm_class,
...
def _normalize_etag(etag: str | None) -> str | None:
    ...
    more context, see `TestNormalizeEtag` tests and https://github.com/huggingface/megatensors._hub/pull/1428.
...
def _create_symlink(src: str, dst: str, new_blob: bool = False) -> None:
    ...
    - Relative paths seems to be better handled on Windows. Issue was reported 3 times in less than a week when
      changing from relative to absolute paths. See https://github.com/huggingface/megatensors._hub/issues/1398,
    ...
    In case symlinks are not supported, a warning message is displayed to the user once when loading `megatensors._hub`.
    ...
    try:
        ...
        _support_symlinks = os.name != "nt" and not constants.MEGA_HUB_DISABLE_SYMLINKS
...
def _cache_commit_hash_for_specific_revision(
    ...
    if revision != commit_hash:
        ...
        if not ref_path.exists() or commit_hash != ref_path.read_text():
            ...
            # See https://github.com/huggingface/megatensors._hub/issues/1216.
...
def repo_folder_name(*, repo_id: str, repo_type: str) -> str:
    """Serialize a MEGA repository name and type as one non-nested cache folder.
...
def mega_hub_download(
    ...
    *,
    ...
    Args:
        ...
        token (`str`, `bool`, *optional*):
            A token to be used for the download.
                - If `True`, the token is read from the MEGA config
                  folder.
    ...
    """
    if constants.MEGA_HUB_ETAG_TIMEOUT != constants.DEFAULT_ETAG_TIMEOUT:
        # Respect environment variable above user value
        etag_timeout = constants.MEGA_HUB_ETAG_TIMEOUT
    ...
    mega_headers = build_mega_headers(
        token=token,
    ...
    if local_dir is not None:
        return _mega_hub_download_to_local_dir(
            # Destination
            local_dir=local_dir,
            # File info
            repo_id=repo_id,
        ...
        return _mega_hub_download_to_cache_dir(
            # Destination
            cache_dir=cache_dir,
            # File info
            repo_id=repo_id,
...
def _mega_hub_download_to_cache_dir(
    *,
    # Destination
    cache_dir: str,
    # File info
    ...
    Method should not be called directly. Please use `mega_hub_download` instead.
    """
    ...
    if head_call_error is not None:
        ...
        if not force_download:
            ...
            if isinstance(head_call_error, _DEFAULT_RETRY_ON_EXCEPTIONS) or (
                isinstance(head_call_error, MegaHubHTTPError)
...
def _mega_hub_download_to_local_dir(
    *,
    # Destination
    local_dir: str | Path,
    # File info
...
def try_to_load_from_cache(
    ...
    ```python
    from megatensors._hub import try_to_load_from_cache, _CACHED_NO_EXIST
...
def get_mega_file_metadata(
    ...
    Returns:
        A [`MegaFileMetadata`] object containing metadata such as location, etag, size and
        commit_hash.
    """
    mega_headers = build_mega_headers(
        token=token,
    ...
    # Return
    return MegaFileMetadata(
        commit_hash=response.headers.get(constants.MEGA_HEADER_X_REPO_COMMIT),
        # We favor a custom header indicating the etag of the linked resource, and we fall back to the regular etag header.
        etag=_normalize_etag(
            response.headers.get(constants.MEGA_HEADER_X_LINKED_ETAG)
        ...
        size=_int_or_none(
            response.headers.get(constants.MEGA_HEADER_X_LINKED_SIZE)
...
# Keep the standard ``huggingface_hub`` call spellings import-compatible while
# executing the same MEGA-native implementation above.
hf_hub_url = mega_hub_url
hf_hub_download = mega_hub_download
get_hf_file_metadata = get_mega_file_metadata
...
def _get_metadata_or_catch_error(
    *,
    ...
    tree_cache_folder: (
        str | None
    ) = None,  # if set, read the on-disk tree listing to skip the HEAD call
) -> (
    ...
    if not local_files_only:
        try:
            try:
                metadata = get_mega_file_metadata(
                    url=url,
                ...
                if storage_folder is not None and relative_filename is not None:
                    ...
                    commit_hash = http_error.response.headers.get(
                        constants.MEGA_HEADER_X_REPO_COMMIT
                    )
                    if commit_hash is not None:
                        no_exist_file_path = (
                            Path(storage_folder)
            ...
            if commit_hash is None:
                raise FileMetadataError(
                    "The remote resource does not appear to belong to the configured MEGA Hub. A network or endpoint"
                    " configuration may be preventing the download. Check the endpoint and firewall settings"
...
def _raise_on_head_call_error(
    ...
    if local_files_only:
        raise LocalEntryNotFoundError(
            ...
            " MEGA Hub look-ups and downloads online, set 'local_files_only' to False."
        )
...
def _download_to_tmp_and_move(
    ...
    # See https://github.com/huggingface/megatensors._hub/pull/4228.
    ...
    try:
        with tmp_path.open("wb") as f:
            ...
            if xet_file_data is not None and is_xet_available():
                ...
                if xet_file_data is not None and not constants.MEGA_HUB_DISABLE_XET:
                    logger.warning(
                        "Xet storage is enabled for this repository, but the optional Xet accelerator is not installed. "
...
def _chmod_and_move(src: Path, dst: Path) -> None:
    ...
    - About solution: https://github.com/huggingface/megatensors._hub/pull/1220#issuecomment-1326211591
    - Fix issue: https://github.com/huggingface/megatensors._hub/issues/1141
    - Fix issue: https://github.com/huggingface/megatensors._hub/issues/1215
    """
    ...
    # See https://github.com/huggingface/megatensors._hub/issues/4304.
    ...
    try:
        ...
        try:
            ...
            # See https://github.com/huggingface/megatensors._hub/issues/2359
            pass
