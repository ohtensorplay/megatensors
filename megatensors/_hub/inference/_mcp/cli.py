...
@click.group(
    help="A squad of lightweight composable AI applications built on MEGA's Inference Client and MCP stack."
)
def app() -> None:
    pass
...
async def run_agent(
    ...
    Args:
        agent_path (`str`, *optional*):
            Path to a local folder containing an `agent.json` and optionally a custom `PROMPT.md` or `AGENTS.md` file or a built-in agent stored in a MEGA dataset.
...
def run(path: Optional[str]) -> None:
    ...
    stored in the 'tiny-agents/tiny-agents' MEGA dataset
    (https://mega.tensorplay.cn/datasets/tiny-agents/tiny-agents).
    """
    try:
