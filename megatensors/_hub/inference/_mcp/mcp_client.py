...
from ...utils._runtime import get_mega_hub_version
...
class MCPClient:
    ...
    Args:
        ...
        provider (`str`, *optional*):
            Name of the provider to use for inference. Defaults to "auto" i.e. the first of the providers available for the model, sorted by the user's order in https://mega.tensorplay.cn/settings/inference-providers.
            If model is a URL or `base_url` is passed, then `provider` is not used.
        base_url (`str`, *optional*):
            The base URL to run inference. Defaults to None.
        api_key (`str`, `optional`):
            Token to use for authentication. Will default to the locally MEGA saved token if not provided. You can also use your own provider API key to interact directly with the provider's service.
    """
        ...
        session = await self.exit_stack.enter_async_context(
            ClientSession(
                ...
                client_info=mcp_types.Implementation(
                    name="megatensors._hub.MCPClient",
                    version=get_mega_hub_version(),
                ),
            )
        )
