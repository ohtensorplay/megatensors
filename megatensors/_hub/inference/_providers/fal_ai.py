...
from megatensors._hub.inference._common import RequestParameters, _as_dict, _as_url
...
from megatensors._hub.utils.logging import get_logger
...
class FalAIQueueTask(TaskProviderHelper, ABC):
    ...
    def _prepare_route(self, mapped_model: str, api_key: str) -> str:
        if api_key.startswith("mega_"):
            # Use the queue subdomain for MEGA routing
    ...
    def get_response(
        ...
        base_url = f"{parsed_url.scheme}://{parsed_url.netloc}{'/fal-ai' if parsed_url.netloc == 'router.mega.co' else ''}"
        ...
        while status != "COMPLETED":
            ...
            mega_raise_for_status(status_response)
...
class FalAITextToImageTask(FalAITask):
    ...
    def _prepare_payload_as_dict(
        ...
        if provider_mapping_info.adapter_weights_path is not None:
            lora_path = constants.MEGA_URL_TEMPLATE.format(
                repo_id=provider_mapping_info.model_id,
                revision="main",
...
class FalAIImageSegmentationTask(FalAIQueueTask):
    ...
    def get_response(
        ...
        if isinstance(image_url, str) and image_url.startswith("data:"):
            ...
            mega_raise_for_status(mask_response)
