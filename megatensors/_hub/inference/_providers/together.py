...
from megatensors._hub.inference._providers._common import (
    BaseConversationalTask,
    BaseTextGenerationTask,
    TaskProviderHelper,
    filter_none,
)
from megatensors._hub.utils import get_session, mega_raise_for_status, logging
...
def _normalize_video_parameters(parameters: dict) -> dict:
    """Map MEGA inference-client conventions onto Together's video API parameter names."""
    parameters = filter_none(parameters)
    if "num_inference_steps" in parameters:
        parameters["steps"] = parameters.pop("num_inference_steps")
...
class TogetherVideoTask(TogetherTask, ABC):
    ...
    def get_response(self, response: bytes | dict, request_params: RequestParameters | None = None) -> Any:
        ...
        mega_raise_for_status(video_response)
