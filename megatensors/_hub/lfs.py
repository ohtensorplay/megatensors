...
def post_lfs_batch_info(
    ...
    Args:
        ...
        token (`str` or `None`):
            An authentication token (see https://mega.tensorplay.cn/settings/token).
    ...
    headers = {
        **LFS_HEADERS,
        **build_mega_headers(token=token),
        **(headers or {}),
    }
...
def lfs_upload(
    ...
    Args:
        ...
        lfs_batch_action (`dict`):
            ...
            more details.
        token (`str`, *optional*):
            An authentication token (see https://mega.tensorplay.cn/settings/token). Used to call the
    ...
    upload_url = fix_mega_endpoint_in_url(upload_action["href"], endpoint=endpoint)
    ...
    if verify_action is not None:
        _validate_lfs_action(verify_action)
        verify_url = fix_mega_endpoint_in_url(verify_action["href"], endpoint)
        verify_resp = http_backoff(
            "POST",
            verify_url,
            headers=build_mega_headers(token=token, headers=headers),
            json={"oid": operation.upload_info.sha256.hex(), "size": operation.upload_info.size},
        )
        mega_raise_for_status(verify_resp)
...
def _upload_multi_part(operation: "CommitOperationAdd", header: dict, chunk_size: int, upload_url: str) -> None:
    ...
    mega_raise_for_status(completion_res)
    ...
    with operation.as_file(with_tqdm=True) as fileobj:
        for part_idx, part_upload_url in enumerate(sorted_parts_urls):
            with SliceFileObj(
                ...
                mega_raise_for_status(part_upload_res)
