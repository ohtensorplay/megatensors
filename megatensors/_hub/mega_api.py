...
from .file_download import DryRunFileInfo, MegaFileMetadata, get_mega_file_metadata, mega_hub_url
...
from .utils import (
    ...
    silent_tqdm,
    validate_mega_hub_args,
)
from .utils import tqdm as mega_tqdm
...
MEGA_USERNAME_PLACEHOLDER = "mega_user"
...
_CREATE_COMMIT_NO_REPO_ERROR_MESSAGE = (
    ...
    " MEGA Hub. Please use `create_repo` if it's not the case."
)
...
def repo_type_and_id_from_mega_id(mega_id: str, hub_url: str | None = None) -> tuple[str | None, str | None, str]:
    """
    Return the repository type and ID from a MEGA repository ID, URI, or URL.

    > [!WARNING]
    > Prefer [`parse_mega_uri`] when the caller needs the structured revision
    > and path components as well.

    Args:
        mega_id (`str`):
            A MEGA repository URL, URI, or ID. Accepted values are:

            - https://mega.tensorplay.cn/<repo_type>/<namespace>/<repo_id>
            - https://mega.tensorplay.cn/<namespace>/<repo_id>
            - mega://<repo_type>/<namespace>/<repo_id>
            - mega://<namespace>/<repo_id>
            - <repo_type>/<namespace>/<repo_id>
            - <namespace>/<repo_id>
            - <repo_id>
        hub_url (`str`, *optional*):
            The MEGA Hub endpoint. Defaults to the configured endpoint.
    ...
    # Check if mega_id is a URL containing the Hub endpoint.
    mega_id_without_protocol = _REGEX_HTTP_PROTOCOL.sub("", mega_id)
    is_mega_url = hub_url_without_protocol in mega_id_without_protocol and "@" not in mega_id

    mega_fs_prefix = "mega://"
    if mega_id.startswith(mega_fs_prefix):
        mega_id = mega_id[len(mega_fs_prefix) :]

    # If it's a URL, strip the endpoint prefix to get the path
    if is_mega_url:
        # Remove protocol if present
        mega_id_normalized = _REGEX_HTTP_PROTOCOL.sub("", mega_id)

        # Remove the hub_url prefix to get the relative path
        if mega_id_normalized.startswith(hub_url_without_protocol):
            # Strip the hub URL and any leading slashes
            mega_id = mega_id_normalized[len(hub_url_without_protocol) :].lstrip("/")

    url_segments = mega_id.split("/")
    is_mega_id = len(url_segments) <= 3
    ...
    if is_mega_url:
        ...
        if len(url_segments) == 3:
            ...
            if url_segments[0] in constants.REPO_TYPES_MAPPING:
                ...
                namespace, repo_id = mega_id.split("/")[-2:]
                repo_type = None
        else:
            # Passed <model_id>
            repo_id = url_segments[0]
        ...
        raise ValueError(f"Unable to retrieve namespace and repository ID from MEGA ID: {mega_id}")
    ...
    if repo_type not in constants.REPO_TYPES_WITH_KERNEL and repo_type != "bucket":
        raise ValueError(f"Unknown `repo_type`: '{repo_type}' ('{input_mega_id}')")
...
def _mega_create_commit(
    ...
    *,
    ...
    """Apply typed commit operations through the MEGA atomic commit protocol."""
    from megatensors.hub import MegaHubClient, MegaHubError

    operations = list(operations)
    additions = [
        operation for operation in operations if isinstance(operation, CommitOperationAdd)
    ]
    native_xet_operations = bool(additions) and all(
        isinstance(operation, (CommitOperationAdd, CommitOperationDelete))
        for operation in operations
    )
    native_xet_sources = all(
        isinstance(operation.path_or_fileobj, (str, Path, bytes))
        for operation in additions
    )
    if native_xet_operations and native_xet_sources:
        # Use the same HF-compatible preupload/CAS/commit protocol as `hf upload`.
        # `hf_xet` chunks and hashes files, uploads only missing chunks, finalizes
        # the Xet shard, and commits a small LFS pointer to Git.  Keep the legacy
        # staging path below for copy operations and arbitrary open file objects.
    ...
    if create_pr:
        raise NotImplementedError("MEGA pull requests are not available yet.")
    active_token: str | bool | None = token if token is not None else api_client.token
    api = MegaHubClient(endpoint=api_client.endpoint, token=active_token)
    refs = api.list_refs(repo_id)
    branch_parent = next((ref.target_revision for ref in refs.branches if ref.name == revision), None)
    if branch_parent is None:
        raise MegaHubError(f"branch not found: {revision}")
    if parent_commit is not None and parent_commit != branch_parent:
        raise MegaHubError("409 branch advanced; refresh the parent revision and retry")
    ...
    try:
        for operation in operations:
            if isinstance(operation, CommitOperationAdd):
                ...
                if isinstance(operation.path_or_fileobj, str):
                    ...
                    suffix = Path(operation.path_in_repo).suffix
                    with tempfile.NamedTemporaryFile(prefix="mega-commit-", suffix=suffix, delete=False) as target:
                        with operation.as_file() as source:
                            while chunk := source.read(1024 * 1024):
                                target.write(chunk)
                        local_path = Path(target.name)
                    temporary_paths.append(local_path)
                payload.append(dict(api._stage_file(
                    repo_id,
                    local_path,
                ...
                continue
            if isinstance(operation, CommitOperationCopy):
                payload.append({
                    "operation": "copy",
                    "path": operation.path_in_repo,
                    "source_repo_id": operation.src_repo_id or repo_id,
                    "source_path": operation.src_path_in_repo,
                    "source_revision": operation.src_revision or revision,
                })
                continue
            if isinstance(operation, CommitOperationDelete):
                if operation.is_folder:
                    if remote_files is None:
                        remote_files = api.list_files(repo_id, revision=branch_parent)
                    prefix = operation.path_in_repo.rstrip("/")
                    payload.extend(
                        {"operation": "delete", "path": file.path}
                        for file in remote_files
                        if file.path == prefix or file.path.startswith(f"{prefix}/")
                    )
                else:
                    payload.append({"operation": "delete", "path": operation.path_in_repo})
                continue
            raise TypeError(f"Unsupported MEGA commit operation: {type(operation).__name__}")
        ...
        if payload:
            committed = api.create_commit(
                ...
                payload,
                revision=revision,
                parent_revision=branch_parent,
                commit_message=commit_message,
                commit_description=commit_description,
            )
            oid = str(committed["revision"])
        else:
            oid = branch_parent
        for addition in additions:
            addition._is_committed = True
        prefix = "" if repo_type == constants.REPO_TYPE_MODEL else f"/{repo_type}s"
        return CommitInfo(
            commit_url=f"{api_client.endpoint}{prefix}/{repo_id}/commit/{oid}",
            commit_message=commit_message,
            commit_description=commit_description,
            oid=oid,
            _endpoint=api_client.endpoint,
        )
    finally:
...
def _mega_service_client(api_client: "MegaApi", token: str | bool | None = None):
    """Build the route-level MEGA client with the high-level client's settings."""
    from megatensors.hub import MegaHubClient

    active_token = token if token is not None else api_client.token
    return MegaHubClient(endpoint=api_client.endpoint, token=active_token)


def _native_discussion(info: Any) -> Discussion:
    """Adapt the native route model to the long-lived HF-compatible model."""
    return Discussion(
        title=info.title,
        status=info.status,
        num=info.number,
        ...
        is_pull_request=info.is_pull_request,
        created_at=parse_datetime(info.created_at),
        endpoint=info.endpoint,
    )


def _native_discussion_details(thread: Any) -> DiscussionWithDetails:
    info = thread.discussion
    pull_request = info.pull_request
    blocked_reason = thread.permissions.merge_blocked_reason
    return DiscussionWithDetails(
        ...
        is_pull_request=info.is_pull_request,
        created_at=parse_datetime(info.created_at),
        events=[_native_message_event(message) for message in thread.messages],
        conflicting_files=(True if blocked_reason else []) if pull_request is not None else None,
        target_branch=pull_request.target_branch if pull_request is not None else None,
        merge_commit_oid=(
            pull_request.source_revision
...
def _native_message_event(message: Any, *, hidden: bool = False) -> DiscussionComment:
    latest = {
        "raw": "" if hidden else message.body,
        "html": "" if hidden else message.body,
        "updatedAt": message.updated_at,
        "author": {"name": message.author.handle},
    }
    event = {
    ...
    return deserialize_event(event)  # type: ignore[return-value]


def _native_status_event(thread: Any) -> DiscussionStatusChange:
    info = thread.discussion
    return DiscussionStatusChange(
        id=f"status-{info.number}-{info.updated_at}",
        type="status-change",
        created_at=parse_datetime(info.updated_at),
        author=info.author.handle,
        _event={},
...
def _native_title_event(old_title: str, thread: Any) -> DiscussionTitleChange:
    info = thread.discussion
    return DiscussionTitleChange(
        id=f"title-{info.number}-{info.updated_at}",
        type="title-change",
        created_at=parse_datetime(info.updated_at),
        author=info.author.handle,
        _event={},
...
def _raise_mega_service_error(error: Exception) -> None:
    """Translate route-level failures to the public MEGA Hub error hierarchy."""
    from megatensors.hub import MegaHubError

    if not isinstance(error, MegaHubError):
        raise error
    status_code = error.status_code or 500
    url = error.url or constants.ENDPOINT
    request = httpx.Request(error.method or "GET", url)
    response = httpx.Response(status_code, request=request)
    if status_code == 404:
        raise RepositoryNotFoundError(str(error), response=response) from error
    if status_code == 400:
        raise BadRequestError(str(error), response=response) from error
    raise MegaHubHTTPError(str(error), response=response) from error
...
class RepoUrl(str):
    ...
    `RepoUrl` is returned by `MegaApi.create_repo`. Its string value is the
    canonical URL and its parsed repository fields are available as attributes:
    - endpoint (`str`)
    - namespace (`str`)
    - repo_name (`str`)
    - repo_id (`str`)
    ...
    Args:
        ...
        endpoint (`str`, *optional*):
            Endpoint of the Hub. Defaults to <https://mega.tensorplay.cn>.

    Example:
    ```py
    >>> RepoUrl('https://mega.tensorplay.cn/openai-community/gpt2')
    RepoUrl('https://mega.tensorplay.cn/openai-community/gpt2', endpoint='https://mega.tensorplay.cn', repo_type='model', repo_id='openai-community/gpt2')

    >>> RepoUrl('https://hub-ci.mega.co/datasets/dummy_user/dummy_dataset', endpoint='https://hub-ci.mega.co')
    RepoUrl('https://hub-ci.mega.co/datasets/dummy_user/dummy_dataset', endpoint='https://hub-ci.mega.co', repo_type='dataset', repo_id='dummy_user/dummy_dataset')

    >>> RepoUrl('mega://datasets/my-user/my-dataset')
    RepoUrl('mega://datasets/my-user/my-dataset', endpoint='https://mega.tensorplay.cn', repo_type='dataset', repo_id='user/dataset')

    >>> MegaApi.create_repo("dummy_model")
    RepoUrl('https://mega.tensorplay.cn/Wauplin/dummy_model', endpoint='https://mega.tensorplay.cn', repo_type='model', repo_id='Wauplin/dummy_model')
    ```
    ...
    def __new__(cls, url: Any, endpoint: str | None = None):
        url = fix_mega_endpoint_in_url(url, endpoint=endpoint)
    ...
    def __init__(self, url: Any, endpoint: str | None = None) -> None:
        ...
        # Parse MEGA URIs and web URLs, including custom endpoints. If parsing a
        # bare ID fails, retry it as a MEGA URI.
        raw = str(self)
        try:
            parsed = parse_mega_uri(raw, endpoint=self.endpoint)
        except MegaUriError:
            if "://" in raw:
                raise  # it was a URL: the original error is authoritative, don't retry as a bare id
            parsed = parse_mega_uri(f"{constants.MEGA_PROTOCOL}{raw}")
...
class InferenceProviderMapping:
    ...
    model_id: str  # ID of the model on the MEGA Hub
...
class ModelInfo:
    ...
    Attributes:
        ...
        card_data (`ModelCardData`, *optional*):
            Model Card Metadata  as a [`megatensors._hub.repocard_data.ModelCardData`] object.
        ...
        siblings (`list[RepoSibling]`):
            List of [`megatensors._hub.mega_api.RepoSibling`] objects that constitute the model.
    ...
    def __init__(self, **kwargs):
        ...
        if isinstance(mapping, list):
            self.inference_provider_mapping = [
                InferenceProviderMapping(**{**value, "model_id": self.id}) for value in mapping
            ]
        elif isinstance(mapping, dict):
            self.inference_provider_mapping = [
                InferenceProviderMapping(**{**value, "model_id": self.id, "provider": provider})
...
class DatasetInfo:
    ...
    Attributes:
        ...
        card_data (`DatasetCardData`, *optional*):
            Dataset Card Metadata  as a [`megatensors._hub.repocard_data.DatasetCardData`] object.
        ...
        sha (`str`):
            Repo SHA at this particular revision.
        siblings (`list[RepoSibling]`):
            List of [`megatensors._hub.mega_api.RepoSibling`] objects that constitute the dataset.
        tags (`list[str]`):
...
class SpaceInfo:
    ...
    Attributes:
        ...
        card_data (`SpaceCardData`, *optional*):
            Space Card Metadata  as a [`megatensors._hub.repocard_data.SpaceCardData`] object.
        created_at (`datetime`, *optional*):
            Date of creation of the repo on the Hub. Note that the lowest value is `2022-03-02T23:29:04.000Z`,
            corresponding to the date when we began to store creation dates.
        ...
        runtime (`SpaceRuntime`, *optional*):
            Space runtime information as a [`megatensors._hub.mega_api.SpaceRuntime`] object.
        sdk (`str`, *optional*):
            SDK used by the Space.
        sha (`str`, *optional*):
            Repo SHA at this particular revision.
        siblings (`list[RepoSibling]`):
            List of [`megatensors._hub.mega_api.RepoSibling`] objects that constitute the Space.
...
def _paper_info_from_mega_payload(payload: dict[str, Any]) -> PaperInfo:
    """Adapt MEGA's source-grounded paper records to the public client model."""
    detail = payload.get("paper")
    if isinstance(detail, dict) and detail.get("arxiv_id"):
        record = detail
        daily = payload.get("daily") if isinstance(payload.get("daily"), dict) else {}
        normalized = _normalized_mega_paper(record, daily=daily)
        repositories = payload.get("repos") if isinstance(payload.get("repos"), list) else []
        linked = {"model": [], "dataset": [], "space": []}
        for repository in repositories:
            if not isinstance(repository, dict):
                continue
            repo_id = repository.get("repo_id") or repository.get("id")
            repo_type = repository.get("repo_type") or repository.get("type")
            if isinstance(repo_id, str) and repo_type in linked:
                linked[repo_type].append({"id": repo_id})
        normalized.update(
            linkedModels=linked["model"],
            numTotalModels=len(linked["model"]),
            linkedDatasets=linked["dataset"],
            numTotalDatasets=len(linked["dataset"]),
            linkedSpaces=linked["space"],
        )
        return PaperInfo(**normalized)
    if payload.get("arxiv_id"):
        supplement = payload.get("huggingface") if isinstance(payload.get("huggingface"), dict) else {}
        submission = payload.get("submission") if isinstance(payload.get("submission"), dict) else {}
        return PaperInfo(**_normalized_mega_paper(payload, daily=supplement, submission=submission))
    return PaperInfo(**payload)


def _normalized_mega_paper(
    record: dict[str, Any],
    *,
    daily: dict[str, Any],
    ...
    authors = record.get("official_authors") or record.get("authors") or []
    author_records = [{"name": author} for author in authors if isinstance(author, str)]
    submitter = submission.get("submitter") if submission and isinstance(submission.get("submitter"), dict) else None
    organization = daily.get("organization") if isinstance(daily.get("organization"), dict) else None
    sources = record.get("sources")
    source = record.get("metadata_source") or (
        sources[0] if isinstance(sources, list) and sources else "arxiv"
    )
    return {
        "id": arxiv_id,
        ...
        "paper": {
            ...
            "upvotes": daily.get("upvotes", 0),
        },
        "source": source,
        "numComments": daily.get("comments", 0),
        "submittedOnDailyAt": submission.get("submitted_at") if submission else daily.get("submitted_at"),
        "submittedOnDailyBy": _mega_paper_user(submitter),
        "organization": _mega_paper_organization(organization),
        "projectPage": record.get("project_url") or daily.get("project_url"),
        "githubRepo": record.get("github_url") or daily.get("github_url"),
        "githubStars": daily.get("github_stars", 0),
    }


def _mega_paper_user(value: dict[str, Any] | None) -> dict[str, Any] | None:
    if not value:
        return None
    handle = value.get("handle") or value.get("name")
    if not isinstance(handle, str) or not handle:
        return None
    return {
        "user": handle,
        "fullname": value.get("display_name") or value.get("fullname") or handle,
        "avatarUrl": value.get("avatar_url") or value.get("avatarUrl"),
    }


def _mega_paper_organization(value: dict[str, Any] | None) -> dict[str, Any] | None:
    if not value or not isinstance(value.get("name"), str):
        return None
    return {
        "name": value["name"],
        "fullname": value.get("display_name") or value.get("fullname") or value["name"],
        "avatar": value.get("avatar_url") or value.get("avatar"),
    }
...
class LFSFileInfo:
    ...
    making it impractical to search and resolve these connections. Check out [our documentation](https://mega.tensorplay.cn/docs/hub/storage-limits#advanced-track-lfs-file-references)
...
def future_compatible(fn: CallableT) -> CallableT:
    """Wrap a method of `MegaApi` to handle `run_as_future=True`.
...
class MegaApi:
    """
    Client to interact with the MEGA Hub via HTTP.

    The client is initialized with some high-level settings used in all requests
    made to the Hub (MEGA endpoint, authentication, user agents...). Using the `MegaApi`
    client is preferred but not mandatory as all of its public methods are exposed
    directly at the root of `megatensors._hub`.
    ...
    Args:
        ...
        token (`bool` or `str`, *optional*):
            ...
            https://mega.tensorplay.cn/docs/megatensors._hub/quick-start#authentication).
    ...
    def __init__(
        ...
        # /api/whoami is the identity endpoint for which we may want to cache results
    ...
    def whoami(self, token: bool | str | None = None, *, cache: bool = False) -> dict:
        """
        Call MEGA Hub API to know "whoami".
        ...
        if token is None:
            raise LocalTokenNotFoundError(
                "Token is required to call the MEGA identity endpoint, but no token found. "
                "Provide a token or log in with `mega auth login` or `megatensors._hub.login`."
            )
    ...
    def _inner_whoami(self, token: str) -> dict:
        r = get_session().get(
            f"{self.endpoint}/api/whoami",
            headers=self._build_mega_headers(token=token),
        )
        try:
            mega_raise_for_status(r)
        except MegaHubHTTPError as e:
            if e.response.status_code == 401:
                ...
                if token == _get_token_from_google_colab():
                    ...
                    error_message += (
                        " The token from MEGA_TOKEN environment variable is invalid. "
                        "Note that MEGA_TOKEN takes precedence over `mega auth login`."
                    )
                elif token == _get_token_from_file():
                    error_message += (
                        " The stored token is invalid. Please run `mega auth login --force` to set a new token."
                    )
                raise MegaHubHTTPError(error_message, response=e.response) from e
            if e.response.status_code == 429:
                error_message = (
                    "You've hit the rate limit for the MEGA identity endpoint, which is intentionally strict for security reasons."
    ...
    def list_models(
        ...
        *,
        ...
        """
        List models hosted on the MEGA Hub, given some filters.
        ...
        Returns:
            `Iterable[ModelInfo]`: an iterable of [`megatensors._hub.mega_api.ModelInfo`] objects.
    ...
    def list_datasets(
        ...
        *,
        ...
        """
        List datasets hosted on the MEGA Hub, given some filters.
        ...
        Returns:
            `Iterable[DatasetInfo]`: an iterable of [`megatensors._hub.mega_api.DatasetInfo`] objects.
    ...
    def list_dataset_parquet_files(
        ...
        *,
        ...
        [Dataset Viewer](https://mega.tensorplay.cn/docs/dataset-viewer/parquet).
        ...
        Example:
            ```python
            >>> from megatensors._hub import list_dataset_parquet_files
            >>> list_dataset_parquet_files("lhoestq/demo1")
            >>> entries[0]
            DatasetParquetEntry(config='default', split='train', url='https://mega.tensorplay.cn/...', size=5038)
            ```
        """
        if self.endpoint != constants._MEGA_DEFAULT_ENDPOINT:
            raise ValueError(
                "The Dataset Viewer is only available on the MEGA Hub"
                f" (endpoint='{constants._MEGA_DEFAULT_ENDPOINT}'). It is not supported on"
        ...
        response = get_session().get(url, params=params, headers=self._build_mega_headers(token=token))
    ...
    def list_spaces(
        ...
        *,
        ...
        """
        List spaces hosted on the MEGA Hub, given some filters.
        ...
        Returns:
            `Iterable[SpaceInfo]`: an iterable of [`megatensors._hub.mega_api.SpaceInfo`] objects.
        """
        if expand and full:
            raise ValueError("`expand` cannot be used if `full` is passed.")
    ...
    def unlike(
        ...
        *,
        ...
        Example:
        ```python
        >>> from megatensors._hub import list_liked_repos, unlike
        ...
        response = get_session().delete(
            url=f"{self.endpoint}/api/{repo_type}s/{repo_id}/like", headers=self._build_mega_headers(token=token)
        )
        mega_raise_for_status(response)
    ...
    def list_liked_repos(
        ...
        *,
        ...
        Example:
        ```python
        >>> from megatensors._hub import list_liked_repos
    ...
    def list_user_repos(
        ...
        *,
        ...
        Example:
        ```python
        >>> from megatensors._hub import list_user_repos
    ...
    def list_repo_likers(
        ...
        *,
        ...
        Returns:
            `Iterable[User]`: an iterable of [`megatensors._hub.mega_api.User`] objects.
        """
        ...
        for liker in paginate(path, params={}, headers=self._build_mega_headers(token=token)):
    ...
    def model_info(
        ...
        *,
        ...
        Returns:
            [`megatensors._hub.mega_api.ModelInfo`]: The model repository information.
    ...
    def dataset_info(
        ...
        *,
        ...
        Returns:
            [`mega_api.DatasetInfo`]: The dataset repository information.
    ...
    def space_info(
        ...
        *,
        ...
        Returns:
            [`~mega_api.SpaceInfo`]: The space repository information.
    ...
    def kernel_info(
        ...
        *,
        ...
        Returns:
            [`~mega_api.ModelInfo`]: The kernel repository information.
        """
        headers = self._build_mega_headers(token=token)
        path = (
    ...
    def repo_info(
        ...
        *,
        ...
        Returns:
            ...
            [`megatensors._hub.mega_api.DatasetInfo`], [`megatensors._hub.mega_api.ModelInfo`]
            or [`megatensors._hub.mega_api.SpaceInfo`] object.
        ...
        api = _mega_service_client(self, token)
        try:
            repo = api.repo_info(repo_id)
            if repo_type is not None and repo.repo_type != repo_type:
                raise ValueError(f"Repository '{repo_id}' is a {repo.repo_type}, not a {repo_type}.")
            commit = api.get_commit(repo_id, revision or constants.DEFAULT_REVISION)
            siblings = None
            if files_metadata:
                siblings = [
                    {
                    ...
                    for file in api.list_files(repo_id, revision=commit.revision)
                ]
        except Exception as error:
            _raise_mega_service_error(error)
        data = {
            "id": repo.repo_id,
            "author": repo.owner or repo.repo_id.split("/", 1)[0],
            "sha": commit.revision,
    ...
    def repo_exists(
        ...
        *,
        ...
        """
        Checks if a repository exists on the MEGA Hub.
        ...
        Examples:
            ```py
            >>> from megatensors._hub import repo_exists
    ...
    def revision_exists(
        ...
        *,
        ...
        """
        Checks if a specific revision exists on a repo on the MEGA Hub.
        ...
        Examples:
            ```py
            >>> from megatensors._hub import revision_exists
    ...
    def file_exists(
        ...
        *,
        ...
        """
        Checks if a file exists in a repository on the MEGA Hub.
        ...
        Examples:
            ```py
            >>> from megatensors._hub import file_exists
        ...
        try:
            if token is None:
                token = self.token
            get_mega_file_metadata(url, token=token)
            return True
        ...
        *,
        ...
        Args:
            ...
            expand (`bool`, *optional*, defaults to `False`):
                ...
                As pagination is implemented in `megatensors._hub`, this is transparent for you except for the time it
        ...
        Examples:
            ...
            ```py
            >>> from megatensors._hub import list_repo_tree
            >>> repo_tree = list_repo_tree("lysandre/arxiv-nlp")
            >>> repo_tree
            <generator object MegaApi.list_repo_tree at 0x7fa4088e1ac0>
            >>> list(repo_tree)
            [
        ...
        _ = (repo_type, expand)
        active_token: str | bool | None = token if token is not None else self.token
        prefix = (path_in_repo or "").strip("/")
        files = MegaHubClient(endpoint=self.endpoint, token=active_token).list_files(
            repo_id,
            revision=revision or constants.DEFAULT_REVISION,
        )
        if recursive:
            for file in files:
                if not prefix or file.path == prefix or file.path.startswith(f"{prefix}/"):
                    yield RepoFile(path=file.path, size=file.size, oid=file.sha256)
            return

        folders: set[str] = set()
        for file in files:
            if prefix:
                if not file.path.startswith(f"{prefix}/"):
                    continue
                relative = file.path[len(prefix) + 1 :]
            else:
                relative = file.path
            head, separator, _tail = relative.partition("/")
            if separator:
                folder = f"{prefix}/{head}" if prefix else head
                if folder not in folders:
                    folders.add(folder)
                    yield RepoFolder(path=folder, oid=folder)
            else:
                yield RepoFile(path=file.path, size=file.size, oid=file.sha256)
    ...
    def list_repo_refs(
        ...
        *,
        ...
        _ = repo_type
        active_token: str | bool | None = token if token is not None else self.token
        refs = MegaHubClient(endpoint=self.endpoint, token=active_token).list_refs(repo_id)
        def _format_as_git_ref_info(item: Any) -> GitRefInfo:
            return GitRefInfo(name=item.name, ref=item.ref, target_commit=item.target_revision)
        return GitRefs(
            branches=[_format_as_git_ref_info(item) for item in refs.branches],
            converts=[],
            tags=[_format_as_git_ref_info(item) for item in refs.tags],
    ...
    def list_repo_commits(
        ...
        *,
        ...
        _ = (repo_type, formatted)
        active_token: str | bool | None = token if token is not None else self.token
        api = MegaHubClient(endpoint=self.endpoint, token=active_token)
        commits = []
        cursor = None
        while True:
            page, cursor = api.list_commits(
                repo_id,
                revision=revision or constants.DEFAULT_REVISION,
                limit=100,
                cursor=cursor,
    ...
    def get_paths_info(
        ...
        *,
        ...
        Example:
        ```py
        >>> from megatensors._hub import get_paths_info
        ...
        for requested in paths if isinstance(paths, list) else [paths]:
            normalized = requested.strip("/")
            if file := by_path.get(normalized):
                result.append(RepoFile(path=file.path, size=file.size, oid=file.sha256))
            elif normalized and any(file.path.startswith(f"{normalized}/") for file in files):
                result.append(RepoFolder(path=normalized, oid=normalized))
        return result
    ...
    def super_squash_history(
        ...
        *,
        ...
        headers = self._build_mega_headers(token=token)
        commit_message = commit_message or f"Super-squash branch '{branch}' using megatensors._hub"
    ...
    def list_space_templates(self, *, token: str | bool | None = None) -> list[SpaceTemplate]:
        ...
        to [`MegaApi.create_repo`] to seed a new Space from that template.
        ...
        Example:
            ```py
            >>> from megatensors._hub import list_space_templates
            ...
            ```
        """
        r = get_session().get(f"{self.endpoint}/api/spaces/templates", headers=self._build_mega_headers(token=token))
        mega_raise_for_status(r)
    ...
    def create_repo(
        ...
        *,
        ...
        """Create an empty repo on the MEGA Hub.
        ...
        Args:
            ...
            resource_group_id (`str`, *optional*):
                ...
                can be found in the URL of the resource's page on the Hub
                (e.g. `"9a70e516-3145-4ca5-a2cb-1988c8d9e001"`).
                To learn more about resource groups, see
                https://mega.tensorplay.cn/docs/hub/organizations#resource-groups.
            ...
            space_sleep_time (`int`, *optional*):
                ...
                See https://mega.tensorplay.cn/docs/hub/spaces-gpus#sleep-time for more details.
            space_secrets (`list[dict[str, str]]`, *optional*):
                ...
                For more details, see https://mega.tensorplay.cn/docs/hub/spaces-overview#managing-secrets.
            space_variables (`list[dict[str, str]]`, *optional*):
                ...
                For more details, see https://mega.tensorplay.cn/docs/hub/spaces-overview#managing-secrets-and-environment-variables.
            ...
            space_template (`str`, *optional*):
                ...
                [`MegaApi.list_space_templates`] to list available templates. Only applicable if repo_type is
        ...
        """
        resolved_type = repo_type or constants.REPO_TYPE_MODEL
        if resolved_type not in constants.REPO_TYPES:
            raise ValueError(f"Invalid repo type: {resolved_type!r}")
        resolved_visibility = _resolve_repo_visibility(
            private=private,
            visibility=visibility,
            repo_type=resolved_type,
        )
        if resolved_visibility == "protected":
            raise NotImplementedError("Protected repository visibility is not available on MEGA Hub yet.")
        unsupported = {
            ...
            "space_volumes": space_volumes,
            "space_template": space_template,
        }
        requested_unsupported = [name for name, value in unsupported.items() if value is not None]
        if requested_unsupported:
            raise NotImplementedError(
                "MEGA Hub does not support these repository creation fields yet: "
                + ", ".join(requested_unsupported)
            )
        try:
            repo = _mega_service_client(self, token).create_repo(
                repo_id,
                repo_type=resolved_type,
                private=resolved_visibility == "private",
                exist_ok=exist_ok,
            )
        except Exception as error:
            _raise_mega_service_error(error)
        prefix = constants.REPO_TYPES_URL_PREFIXES.get(resolved_type, "")
        return RepoUrl(
            f"{self.endpoint}/{prefix}{repo.repo_id}",
        ...
        if space_template is not None:
            ...
            for candidate in all_templates:
                ...
                raise ValueError(
                    f"Unknown Space template '{space_template}'. Expected one of {', '.join(sorted(t.name for t in all_templates))}. Use `MegaApi.list_space_templates()` to list templates."
                )
        ...
        try:
            mega_raise_for_status(r)
        except MegaHubHTTPError as err:
    ...
    def delete_repo(
        ...
        *,
        ...
        """
        Delete a repo from the MEGA Hub. CAUTION: this is irreversible.
        ...
        """
        if repo_type not in (None, *constants.REPO_TYPES):
            raise ValueError(f"Invalid repo type: {repo_type!r}")
        from megatensors.hub import MegaHubError
            ...
            if repo_type is not None:
                ...
                if repo.repo_type != repo_type:
                    raise RepositoryNotFoundError(
                        f"Repository '{repo_id}' is not a {repo_type} repository."
                    )
    ...
    def update_repo_settings(
        ...
        *,
        ...
        Raises:
            ...
            [`~utils.MegaHubHTTPError`]:
                If the request to the MEGA Hub API fails.
            [`~utils.RepositoryNotFoundError`]
                If the repository to download from cannot be found. This may be because it doesn't exist,
                or because it is set to `private` and you do not have access.
        """
        ...
        if resolved_type not in constants.REPO_TYPES:
            raise ValueError(f"Invalid repo type: {resolved_type!r}")
        if gated is not None:
            raise NotImplementedError("Gated repository settings are not available on MEGA Hub yet.")
        resolved_visibility = _resolve_repo_visibility(
            private=private,
            visibility=visibility,
            repo_type=resolved_type,
        ...
        if resolved_visibility is None:
            raise ValueError("Provide `private` or `visibility` to update repository settings.")
        if resolved_visibility == "protected":
            raise NotImplementedError("Protected repository visibility is not available on MEGA Hub yet.")
        try:
            api = _mega_service_client(self, token)
            repo = api.repo_info(repo_id)
            if repo.repo_type != resolved_type:
                raise ValueError(f"Repository '{repo_id}' is a {repo.repo_type}, not a {resolved_type}.")
            api.update_repo(repo_id, private=resolved_visibility == "private")
            return
        except Exception as error:
            _raise_mega_service_error(error)
    ...
    def move_repo(
        ...
        *,
        ...
        https://mega.tensorplay.cn/docs/hub/repositories-settings#renaming-or-transferring-a-repo.
        ...
        if len(from_id.split("/")) != 2:
            raise ValueError(f"Invalid repo_id: {from_id}. Expected 'namespace/name'.")
        if len(to_id.split("/")) != 2:
            raise ValueError(f"Invalid repo_id: {to_id}. Expected 'namespace/name'.")
        if repo_type not in (None, *constants.REPO_TYPES):
            raise ValueError(f"Invalid repo type: {repo_type!r}")
        try:
            api = _mega_service_client(self, token)
            repo = api.repo_info(from_id)
            if repo_type is not None and repo.repo_type != repo_type:
                raise ValueError(f"Repository '{from_id}' is a {repo.repo_type}, not a {repo_type}.")
            api.move_repo(from_id, to_id)
            return
        except Exception as error:
            _raise_mega_service_error(error)
    ...
    def create_commit(
        ...
        *,
        ...
        > create it first using [`~mega_api.create_repo`].
        ...
        Args:
            ...
            operations (`Iterable` of [`~mega_api.CommitOperation`]):
                An iterable of operations to include in the commit, either:

                    - [`~mega_api.CommitOperationAdd`] to upload a file
                    - [`~mega_api.CommitOperationDelete`] to delete a file
                    - [`~mega_api.CommitOperationCopy`] to copy a file
    ...
    def preupload_lfs_files(
        ...
        *,
        ...
        Example:
        ```py
        >>> from megatensors._hub import CommitOperationAdd, preupload_lfs_files, create_commit, create_repo
    ...
    def upload_file(
        ...
        *,
        ...
        >       if the MEGA API returned an error
        >     - [`ValueError`](https://docs.python.org/3/library/exceptions.html#ValueError)
        ...
        ```python
        >>> from megatensors._hub import upload_file
        ...
        commit_message = (
            commit_message if commit_message is not None else f"Upload {path_in_repo} with megatensors._hub"
        )
    ...
    def upload_folder(
        ...
        *,
        ...
        Files are staged through MEGA's resumable upload routes and then published
        together through the atomic commit API. A failed compare-and-swap leaves the
        branch unchanged and can be retried after refreshing its parent revision.
        ...
        Args:
            ...
            commit_message (`str`, *optional*):
                ...
                `f"Upload {path_in_repo} with megatensors._hub"`
            commit_description (`str` *optional*):
                The description of the generated commit
            create_pr (`boolean`, *optional*):
        ...
        > are set correctly. If repo does not exist, create it first using [`~mega_api.create_repo`].
        ...
        commit_message = commit_message or "Upload folder using megatensors._hub"
    ...
    def delete_file(
        ...
        *,
        ...
        Args:
            ...
            commit_message (`str`, *optional*):
                The summary / title / first line of the generated commit. Defaults to
                `f"Delete {path_in_repo} with megatensors._hub"`.
            commit_description (`str` *optional*)
                The description of the generated commit
            create_pr (`boolean`, *optional*):
                Whether or not to create a Pull Request with that commit. Defaults to `False`.
        ...
        """
        commit_message = (
            commit_message if commit_message is not None else f"Delete {path_in_repo} with megatensors._hub"
        )
        ...
        *,
        ...
        Args:
            ...
            commit_message (`str`, *optional*):
                ...
                `f"Delete files using megatensors._hub"`.
            commit_description (`str` *optional*)
                The description of the generated commit.
            create_pr (`boolean`, *optional*):
                Whether or not to create a Pull Request with that commit. Defaults to `False`.
        ...
        if commit_message is None:
            commit_message = f"Delete files {' '.join(delete_patterns)} with megatensors._hub"
    ...
    def delete_folder(
        ...
        *,
        ...
        Args:
            ...
            commit_message (`str`, *optional*):
                The summary / title / first line of the generated commit. Defaults to
                `f"Delete folder {path_in_repo} with megatensors._hub"`.
            commit_description (`str` *optional*)
                The description of the generated commit.
            create_pr (`boolean`, *optional*):
                Whether or not to create a Pull Request with that commit. Defaults to `False`.
        ...
        return self.create_commit(
            ...
            revision=revision,
            commit_message=(
                commit_message if commit_message is not None else f"Delete folder {path_in_repo} with megatensors._hub"
            ),
            commit_description=commit_description,
            create_pr=create_pr,
            parent_commit=parent_commit,
    ...
    def upload_large_folder(
        ...
        *,
        ...
        At each step, some metadata information about the upload process is saved in the folder under `.cache/.mega/`
        ...
        Args:
            repo_id (`str`):
                ...
                E.g. `"MEGATB/smollm-corpus"`.
            folder_path (`str` or `Path`):
                Path to the folder to upload on the local file system.
            repo_type (`str`):
                Type of the repository. Must be one of `"model"`, `"dataset"` or `"space"`.
                Unlike in all other `MegaApi` methods, `repo_type` is explicitly required here. This is to avoid
        ...
        >     - Repository limits still apply: https://mega.tensorplay.cn/docs/hub/repositories-recommendations
        >     - Do not start several processes in parallel.
        >     - You can interrupt and resume the process at any time.
        >     - Do not upload the same folder to several repositories. If you need to do so, you must delete the local `.cache/.mega/` folder first.
    ...
    @validate_mega_hub_args
    def get_mega_file_metadata(
        self,
        *,
        url: str,
        token: bool | str | None = None,
        timeout: float | None = constants.MEGA_HUB_ETAG_TIMEOUT,
    ) -> MegaFileMetadata:
        """Fetch metadata of a file versioned on the Hub for a given url.

        Args:
            url (`str`):
                File url, for example returned by [`mega_hub_url`].
            token (`bool` or `str`, *optional*):
                A valid user access token (string). Defaults to the locally saved
                token, which is the recommended method for authentication (see
                https://mega.tensorplay.cn/docs/megatensors._hub/quick-start#authentication).
        ...
        Returns:
            A [`MegaFileMetadata`] object containing metadata such as location, etag, size and commit_hash.
        """
        if token is None:
            # Cannot do `token = token or self.token` as token can be `False`.
            token = self.token

        return get_mega_file_metadata(
            url=url,
            token=token,
            timeout=timeout,
    ...
    def mega_hub_download(
        ...
        *,
        ...
        option, the `cache_dir` will not be used and a `.cache/mega/` folder will be created at the root of `local_dir`
        ...
        Args:
            ...
            tqdm_class (`tqdm`, *optional*):
                ...
                Defaults to the custom MEGA progress bar that can be disabled by setting
                `MEGA_HUB_DISABLE_PROGRESS_BARS` environment variable.
        ...
        Raises:
            ...
            [`ValueError`](https://docs.python.org/3/library/exceptions.html#ValueError)
                If some parameter value is invalid.
        """
        from .file_download import mega_hub_download

        if token is None:
            # Cannot do `token = token or self.token` as token can be `False`.
            token = self.token

        return mega_hub_download(
            repo_id=repo_id,
            filename=filename,
            subfolder=subfolder,
            repo_type=repo_type,
    ...
    def get_safetensors_metadata(
        ...
        *,
        ...
        For more details regarding the safetensors format, check out https://mega.tensorplay.cn/docs/safetensors/index#format.
        ...
        if self.file_exists(  # Single safetensors file => non-sharded model
            ...
            # Fetch index
            index_file = self.mega_hub_download(
                repo_id=repo_id,
                filename=constants.SAFETENSORS_INDEX_FILE,
                repo_type=repo_type,
                revision=revision,
    ...
    def parse_safetensors_file_metadata(
        ...
        *,
        ...
        _headers = self._build_mega_headers(token=token)
        ...
        # See https://github.com/huggingface/megatensors._hub/pull/1855#discussion_r1404286419.
    ...
    def create_branch(
        ...
        *,
        ...
        _ = repo_type
        active_token: str | bool | None = token if token is not None else self.token
        MegaHubClient(endpoint=self.endpoint, token=active_token).create_branch(
            repo_id,
            branch,
            revision=revision or constants.DEFAULT_REVISION,
            exist_ok=exist_ok,
    ...
    def delete_branch(
        ...
        *,
        ...
        _ = repo_type
        active_token: str | bool | None = token if token is not None else self.token
        MegaHubClient(endpoint=self.endpoint, token=active_token).delete_branch(repo_id, branch)
    ...
    def create_tag(
        ...
        *,
        ...
        _ = repo_type
        active_token: str | bool | None = token if token is not None else self.token
        MegaHubClient(endpoint=self.endpoint, token=active_token).create_tag(
            repo_id,
            tag,
            revision=revision or constants.DEFAULT_REVISION,
            message=tag_message,
    ...
    def delete_tag(
        ...
        *,
        ...
        _ = repo_type
        active_token: str | bool | None = token if token is not None else self.token
        MegaHubClient(endpoint=self.endpoint, token=active_token).delete_tag(repo_id, tag)
    ...
    def get_repo_discussions(
        ...
        *,
        ...
        Example:
            ...
            ```python
            >>> from megatensors._hub import get_repo_discussions
        ...
        client = _mega_service_client(self, token)
        page_number = 1
        try:
            while True:
                page = client.list_discussions(
                    repo_id,
                    status=discussion_status or "all",
                    kind=discussion_type or "all",
                    page=page_number,
                    limit=50,
                    repo_type=repo_type,
                )
                for discussion in page.discussions:
                    if author is not None and discussion.author.handle.casefold() != author.casefold():
                        continue
                    yield _native_discussion(discussion)
    ...
    def get_discussion_details(
        ...
        *,
        ...
        try:
            thread = _mega_service_client(self, token).get_discussion(
                repo_id,
                discussion_num,
                repo_type=repo_type,
            )
            return _native_discussion_details(thread)
        except Exception as error:
            _raise_mega_service_error(error)
    ...
    def create_discussion(
        ...
        *,
        ...
        Creating a Pull Request with changes can also be done at once with [`MegaApi.create_commit`].
        ...
        Args:
            ...
            description (`str`, *optional*):
                ...
                Defaults to `"Discussion opened with the megatensors._hub Python library"`
        ...
        description = (
            ...
            if description
                ...
                " [megatensors._hub Python"
                " library](https://mega.tensorplay.cn/docs/megatensors._hub)"
            )
        )

        if pull_request and not source_branch:
            raise ValueError("source_branch is required when pull_request=True")
        if not pull_request and source_branch is not None:
            raise ValueError("source_branch is only valid when pull_request=True")
        try:
            thread = _mega_service_client(self, token).create_discussion(
                repo_id,
                title=title.strip(),
                body=description,
    ...
    def create_pull_request(
        ...
        *,
        ...
        This is a wrapper around [`MegaApi.create_discussion`].
    ...
    def comment_discussion(
        ...
        *,
        ...
        """
        try:
            thread = _mega_service_client(self, token).reply_to_discussion(
                repo_id,
                discussion_num,
                comment,
                repo_type=repo_type or constants.REPO_TYPE_MODEL,
            )
            return _native_message_event(thread.messages[-1])
        except Exception as error:
            _raise_mega_service_error(error)
            raise AssertionError("unreachable")
    ...
    def rename_discussion(
        ...
        *,
        ...
        try:
            client = _mega_service_client(self, token)
            before = client.get_discussion(
                repo_id,
                discussion_num,
                repo_type=repo_type or constants.REPO_TYPE_MODEL,
            )
            after = client.update_discussion(
                repo_id,
                discussion_num,
                title=new_title,
                repo_type=repo_type or constants.REPO_TYPE_MODEL,
            )
            return _native_title_event(before.discussion.title, after)
        except Exception as error:
            _raise_mega_service_error(error)
            raise AssertionError("unreachable")
    ...
    def change_discussion_status(
        ...
        *,
        ...
        try:
            thread = _mega_service_client(self, token).update_discussion(
                repo_id,
                discussion_num,
                status=new_status,
            ...
            return _native_status_event(thread)
        except Exception as error:
            _raise_mega_service_error(error)
            raise AssertionError("unreachable")
    ...
    def merge_pull_request(
        ...
        *,
        ...
        """
        try:
            thread = _mega_service_client(self, token).merge_pull_request(
                repo_id,
                discussion_num,
                comment=comment.strip() if comment and comment.strip() else None,
                repo_type=repo_type or constants.REPO_TYPE_MODEL,
    ...
    def edit_discussion_comment(
        ...
        *,
        ...
        """
        try:
            thread = _mega_service_client(self, token).edit_discussion_message(
                repo_id,
                discussion_num,
                comment_id,
                new_content,
            ...
            message = next(message for message in thread.messages if message.message_id == comment_id)
            return _native_message_event(message)
        except Exception as error:
            _raise_mega_service_error(error)
            raise AssertionError("unreachable")
    ...
    def hide_discussion_comment(
        ...
        *,
        ...
        try:
            client = _mega_service_client(self, token)
            thread = client.get_discussion(
                repo_id,
                discussion_num,
                repo_type=repo_type or constants.REPO_TYPE_MODEL,
            )
            message = next(message for message in thread.messages if message.message_id == comment_id)
            client.delete_discussion_message(
                repo_id,
                discussion_num,
                comment_id,
                repo_type=repo_type or constants.REPO_TYPE_MODEL,
            )
            return _native_message_event(message, hidden=True)
        except Exception as error:
            _raise_mega_service_error(error)
            raise AssertionError("unreachable")
    ...
    def get_space_secrets(self, repo_id: str, *, token: bool | str | None = None) -> dict[str, SpaceSecret]:
        ...
        {'MEGA_TOKEN': SpaceSecret(key='MEGA_TOKEN', description='...', updated_at=datetime.datetime(...))}
        ```
        """
        r = get_session().get(
            f"{self.endpoint}/api/spaces/{repo_id}/secrets",
    ...
    def get_space_runtime(self, repo_id: str, *, token: bool | str | None = None) -> SpaceRuntime:
        ...
        """
        r = get_session().get(
            f"{self.endpoint}/api/spaces/{repo_id}/runtime", headers=self._build_mega_headers(token=token)
        )
        mega_raise_for_status(r)
    ...
    def list_spaces_hardware(self, token: bool | str | None = None) -> list[JobHardwareInfo]:
        ...
        ```python
        >>> from megatensors._hub import list_spaces_hardware
        ...
        response = get_session().get(
            f"{self.endpoint}/api/spaces/hardware", headers=self._build_mega_headers(token=token)
        )
        mega_raise_for_status(response)
    ...
    def set_space_sleep_time(self, repo_id: str, sleep_time: int, *, token: bool | str | None = None) -> SpaceRuntime:
        ...
        https://mega.tensorplay.cn/docs/hub/spaces-gpus#sleep-time.
    ...
    def pause_space(self, repo_id: str, *, token: bool | str | None = None) -> SpaceRuntime:
        ...
        For more details, please visit [the docs](https://mega.tensorplay.cn/docs/hub/spaces-gpus#pause).
        ...
        """
        r = get_session().post(
            f"{self.endpoint}/api/spaces/{repo_id}/pause", headers=self._build_mega_headers(token=token)
        )
        mega_raise_for_status(r)
        return SpaceRuntime(r.json())
    ...
    def enable_space_dev_mode(self, repo_id: str, *, token: bool | str | None = None) -> SpaceRuntime:
        ...
        a PRO or Team & Enterprise plan. See https://mega.tensorplay.cn/docs/hub/spaces-dev-mode for more details.
    ...
    def restart_space(
        ...
        if factory_reboot:
            params["factory"] = "true"
        r = get_session().post(
            f"{self.endpoint}/api/spaces/{repo_id}/restart", headers=self._build_mega_headers(token=token), params=params
        )
        mega_raise_for_status(r)
        return SpaceRuntime(r.json())
    ...
    def fetch_space_logs(
        ...
        *,
        ...
        Example:
            ...
            ```python
            >>> from megatensors._hub import fetch_space_logs
        ...
        *,
        ...
        Args:
            ...
            token (`bool` or `str`, *optional*):
                ...
                See https://mega.tensorplay.cn/docs/megatensors._hub/quick-start#authentication.
        ...
        Example:
            ...
            ```python
            >>> from megatensors._hub import restart_space, wait_for_space
    ...
    def duplicate_repo(
        ...
        *,
        ...
        RepoUrl('https://mega.tensorplay.cn/spaces/nateraw/dreambooth-training',...)
        ```
        """
        if repo_type not in constants.REPO_TYPES:
            raise ValueError("Invalid repo type")
        ...
        # otherwise we reuse the source name. Both may be a URL, an 'mega://' URI, or a bare id, so we
        # try 'parse_mega_uri' first and fall back to a plain split ('<namespace>/<name>', or just
        # '<name>' for 'to_id'). A missing namespace defaults to the caller's account.
        to_namespace: str | None = None
        if to_id is not None:
            try:
                to_namespace, to_repo_name = parse_mega_uri(to_id).id.split("/")
            except MegaUriError:
            ...
            try:
                to_repo_name = parse_mega_uri(from_id).id.split("/")[1]
            except MegaUriError:
    ...
    def duplicate_space(
        ...
        *,
        ...
        Example:
        ```python
        >>> from megatensors._hub import duplicate_space
        ...
        RepoUrl('https://mega.tensorplay.cn/spaces/nateraw/my-dreambooth',...)
        ```

        > [!WARNING]
        > `duplicate_space` is deprecated and will be removed in version 2.0. Use [`~MegaApi.duplicate_repo`] instead.
        """
        kwargs: dict[str, Any] = {}
        if to_id is not None:
            kwargs["to_id"] = to_id
    ...
    def set_space_volumes(
        ...
        *,
        ...
        Example:
            ```python
            >>> from megatensors._hub import MegaApi, Volume
            >>> api = MegaApi()
        ...
        *,
        ...
        Args:
            ...
            scaling_metric (`str` or [`InferenceEndpointScalingMetric `], *optional*):
                ...
                None (meaning: let the MEGA Endpoints service specify the metric).
            scaling_threshold (`float`, *optional*):
                ...
                Defaults to None (meaning: let the MEGA Endpoints service specify the threshold).
            ...
            ...     repository="MEGAH4/zephyr-7b-beta",
            ...     framework="pytorch",
            ...     task="text-generation",
            ...     accelerator="gpu",
            ...     vendor="aws",
    ...
    def create_inference_endpoint_from_catalog(
        ...
        *,
        ...
        """Create a new Inference Endpoint from a model in the MEGA Inference Catalog.

        The goal of the Inference Catalog is to provide a curated list of models that are optimized for inference
        and for which default configurations have been tested. See https://endpoints.mega.co/catalog for a list
    ...
    def list_inference_catalog(self, *, token: bool | str | None = None) -> list[str]:
        """List models available in the MEGA Inference Catalog.
    ...
    def get_inference_endpoint(
        ...
        'running'
        >>> endpoint.url
        'https://my-text-to-image.region.vendor.endpoints.mega.cloud'
    ...
    def list_collections(
        ...
        *,
        ...
        """List collections on the MEGA Hub, given some filters.
    ...
    def get_collection(self, collection_slug: str, *, token: bool | str | None = None) -> Collection:
        ...
        ```py
        >>> from megatensors._hub import get_collection
        ...
        """
        r = get_session().get(
            f"{self.endpoint}/api/collections/{collection_slug}", headers=self._build_mega_headers(token=token)
        )
        mega_raise_for_status(r)
    ...
    def create_collection(
        ...
        *,
        ...
        ```py
        >>> from megatensors._hub import create_collection
        ...
        r = get_session().post(
            f"{self.endpoint}/api/collections", headers=self._build_mega_headers(token=token), json=payload
        )
        try:
            mega_raise_for_status(r)
        except MegaHubHTTPError as err:
    ...
    def update_collection_metadata(
        ...
        *,
        ...
        ```py
        >>> from megatensors._hub import update_collection_metadata
    ...
    def delete_collection(
        ...
        ```py
        >>> from megatensors._hub import delete_collection
    ...
    def add_collection_item(
        ...
        *,
        ...
        ```py
        >>> from megatensors._hub import add_collection_item
    ...
    def update_collection_item(
        ...
        *,
        ...
        ```py
        >>> from megatensors._hub import get_collection, update_collection_item
    ...
    def delete_collection_item(
        ...
        *,
        ...
        ```py
        >>> from megatensors._hub import get_collection, delete_collection_item
    ...
    def list_pending_access_requests(
        ...
        For more info about gated repos, see https://mega.tensorplay.cn/docs/hub/models-gated.
        ...
        Example:
        ```py
        >>> from megatensors._hub import list_pending_access_requests, accept_access_request
    ...
    def list_accepted_access_requests(
        ...
        Example:
        ```py
        >>> from megatensors._hub import list_accepted_access_requests
    ...
    def list_rejected_access_requests(
        ...
        Example:
        ```py
        >>> from megatensors._hub import list_rejected_access_requests
    ...
    def get_webhook(self, webhook_id: str, *, token: bool | str | None = None) -> WebhookInfo:
        ...
        Args:
            ...
            token (`bool` or `str`, *optional*):
                ...
                method for authentication (see https://mega.tensorplay.cn/docs/megatensors._hub/quick-start#authentication).
                To disable authentication, pass `False`.
        ...
        Example:
            ```python
            >>> from megatensors._hub import get_webhook
            >>> webhook = get_webhook("654bbbc16f2ec14d77f109cc")
            >>> print(webhook)
            WebhookInfo(
                id="654bbbc16f2ec14d77f109cc",
                job=None,
                watched=[WebhookWatchedItem(type="user", name="julien-c"), WebhookWatchedItem(type="org", name="MEGAH4")],
    ...
    def list_webhooks(self, *, token: bool | str | None = None) -> list[WebhookInfo]:
        ...
        Example:
            ```python
            >>> from megatensors._hub import list_webhooks
    ...
    def create_webhook(
        ...
        *,
        ...
        The webhook can either send a payload to a URL, or trigger a Job to run on MEGA infrastructure.
        ...
        Example:
            ...
            ```python
            >>> from megatensors._hub import create_webhook
            >>> payload = create_webhook(
            ...     watched=[{"type": "user", "name": "julien-c"}, {"type": "org", "name": "MEGAH4"}],
            ...     url="https://webhook.site/a2176e82-5720-43ee-9e06-f91cb4c91548",
            ...     domains=["repo", "discussion"],
            ...     secret="my-secret",
            ... )
            ...
            ```python
            >>> from megatensors._hub import create_webhook, run_job
            >>> job = run_job(
            ...     image="ubuntu",
    ...
    def update_webhook(
        ...
        *,
        ...
        Example:
            ```python
            >>> from megatensors._hub import update_webhook
    ...
    def enable_webhook(self, webhook_id: str, *, token: bool | str | None = None) -> WebhookInfo:
        ...
        Example:
            ```python
            >>> from megatensors._hub import enable_webhook
    ...
    def disable_webhook(self, webhook_id: str, *, token: bool | str | None = None) -> WebhookInfo:
        ...
        Example:
            ```python
            >>> from megatensors._hub import disable_webhook
    ...
    def delete_webhook(self, webhook_id: str, *, token: bool | str | None = None) -> None:
        ...
        Example:
            ```python
            >>> from megatensors._hub import delete_webhook
    ...
    def _build_mega_headers(
        ...
        """
        Alias for [`build_mega_headers`] that uses the token from [`MegaApi`] client
        when `token` is not provided.
        """
        if token is None:
            # Cannot do `token = token or self.token` as token can be `False`.
            token = self.token
        return build_mega_headers(
            token=token,
    ...
    def get_user_overview(self, username: str, token: bool | str | None = None) -> User:
        ...
        """
        r = get_session().get(
            f"{constants.ENDPOINT}/api/users/{username}/overview", headers=self._build_mega_headers(token=token)
        )
        mega_raise_for_status(r)
        return User(**r.json())
    ...
    def get_organization_overview(self, organization: str, token: bool | str | None = None) -> Organization:
        ...
        Args:
            ...
            token (`bool` or `str`, *optional*):
                ...
                for authentication (see https://mega.tensorplay.cn/docs/megatensors._hub/quick-start#authentication).
                To disable authentication, pass `False`.
    ...
    def list_papers(
        ...
        *,
        ...
        """
        List daily papers on the MEGA Hub given a search query.
        ...
        Returns:
            `Iterable[PaperInfo]`: an iterable of [`megatensors._hub.mega_api.PaperInfo`] objects.
        ...
        for paper in body.get("papers", []):
            yield _paper_info_from_mega_payload(paper)
    ...
    def paper_info(self, id: str) -> PaperInfo:
        ...
        r = get_session().get(path)
        mega_raise_for_status(r)
        return _paper_info_from_mega_payload(r.json())
    ...
    def read_paper(self, id: str) -> str:
        ...
        Raises:
            [`MegaHubHTTPError`]:
                HTTP 404 If the paper does not exist on the Hub.
        """
        path = f"{self.endpoint}/api/papers/{id}/markdown"
        r = get_session().get(path)
        mega_raise_for_status(r)
        return r.text
    ...
    def list_daily_papers(
        ...
        *,
        ...
        """
        List the daily papers published on a given date on the MEGA Hub.
        ...
        r = get_session().get(path, params=params, headers=self._build_mega_headers(token=token))
        mega_raise_for_status(r)
        body = r.json()
        for paper in body.get("papers", []):
            yield _paper_info_from_mega_payload(paper)
    ...
    def auth_check(
        ...
        *,
        ...
        """
        Check if the provided user token has access to a specific repository on the MEGA Hub.
        ...
        Args:
            ...
            token (`Union[bool, str, None]`, *optional*):
                ...
                recommended authentication method. Set to `False` to disable authentication.
                Refer to: https://mega.tensorplay.cn/docs/megatensors._hub/quick-start#authentication.
        ...
        Example:
            ...
            ```python
            >>> from megatensors._hub import auth_check
            >>> from megatensors._hub.utils import GatedRepoError, RepositoryNotFoundError
    ...
    def run_job(
        ...
        *,
        ...
        """
        Run compute Jobs on MEGA infrastructure.

        Args:
            image (`str`):
                The Docker image to use.
                Examples: `"ubuntu"`, `"python:3.12"`, `"pytorch/pytorch:2.6.0-cuda12.4-cudnn9-devel"`.
                Example with an image from a Space: `"mega.tensorplay.cn/spaces/lhoestq/duckdb"`.
            ...
            volumes (`list[Volume]`, *optional*):
                MEGA Buckets or Repos to mount as volumes in the job container.
            ...
            expose (`list[int]`, *optional*):
                ...
                on the public jobs domain (e.g. `https://<job_id>--8000.mega.jobs`). Access always
                requires an MEGA token with read access to the job's namespace.

            ssh (`bool`, *optional*):
                Enable the private account-key-authenticated Job SSH ingress.
                The running Job returns its endpoint in `job.status.ssh_url`.
        ...
        Example:
            ...
            ```python
            >>> from megatensors._hub import run_job
            ...
            ```python
            >>> from megatensors._hub import Volume, run_job
            >>> dataset_volume = Volume(type="dataset", source="MEGAFW/fineweb", mount_path="/data")
    ...
    def _fetch_running_job_sse(
        ...
        *,
        ...
        def has_job_finished() -> bool:
            ...
            mega_raise_for_status(job_status_response)
    ...
    def fetch_job_logs(
        ...
        *,
        ...
        """
        Fetch all the logs from a compute Job on MEGA infrastructure.
        ...
        Example:
            ...
            ```python
            >>> from megatensors._hub import fetch_job_logs, run_job
    ...
    def fetch_job_metrics(
        ...
        *,
        ...
        """
        Fetch all the live metrics from a compute Job on MEGA infrastructure.
        ...
        Example:
            ...
            ```python
            >>> from megatensors._hub import fetch_job_metrics, run_job
    ...
    def list_jobs(
        ...
        *,
        ...
        """
        List compute Jobs on MEGA infrastructure.
    ...
    def list_jobs_hardware(self, token: bool | str | None = None) -> list[JobHardwareInfo]:
        """
        List available hardware options for Jobs on MEGA infrastructure.
        ...
        ```
        """
        response = get_session().get(f"{self.endpoint}/api/jobs/hardware", headers=self._build_mega_headers(token=token))
        mega_raise_for_status(response)
        return [JobHardwareInfo(**hardware) for hardware in response.json()]
    ...
    def inspect_job(
        ...
        *,
        ...
        """
        Inspect a compute Job on MEGA infrastructure.
        ...
        Example:
            ...
            ```python
            >>> from megatensors._hub import inspect_job, run_job
            >>> job = run_job(image="python:3.12", command=["python", "-c" ,"print('Hello from HF compute!')"])
            >>> inspect_job(job.id)
            JobInfo(
    ...
    def wait_for_job(
        ...
        *,
        ...
        """
        Wait until one or more compute Jobs on MEGA infrastructure reach a given stage.
        ...
        Example:
            ...
            ```python
            >>> from megatensors._hub import run_job, wait_for_job
            >>> job = run_job(image="python:3.12", command=["python", "-c", "print('Hello from HF compute!')"])
    ...
    def cancel_job(
        ...
        *,
        ...
        """
        Cancel a compute Job on MEGA infrastructure.
    ...
    def run_uv_job(
        ...
        *,
        ...
        """
        Run a UV script Job on MEGA infrastructure.
        ...
        Example:
            ...
            ```python
            >>> from megatensors._hub import run_uv_job
            ...
            ```python
            >>> from megatensors._hub import Volume, run_uv_job
            >>> script = "my_sft.py"
    ...
    def create_scheduled_job(
        ...
        *,
        ...
        """
        Create scheduled compute Jobs on MEGA infrastructure.
        ...
        Example:
            ...
            ```python
            >>> from megatensors._hub import create_scheduled_job
        ...
        # prepare payload to send to MEGA Jobs API
        job_spec = _create_job_spec(
            image=image,
            command=command,
            env=env,
    ...
    def list_scheduled_jobs(
        ...
        *,
        ...
        """
        List scheduled compute Jobs on MEGA infrastructure.
    ...
    def inspect_scheduled_job(
        ...
        *,
        ...
        """
        Inspect a scheduled compute Job on MEGA infrastructure.
        ...
        Example:
            ...
            ```python
            >>> from megatensors._hub import inspect_job, create_scheduled_job
    ...
    def delete_scheduled_job(
        ...
        *,
        ...
        """
        Delete a scheduled compute Job on MEGA infrastructure.
    ...
    def suspend_scheduled_job(
        ...
        *,
        ...
        """
        Suspend (pause) a scheduled compute Job on MEGA infrastructure.
    ...
    def resume_scheduled_job(
        ...
        *,
        ...
        """
        Resume (unpause) a scheduled compute Job on MEGA infrastructure.
    ...
    def create_scheduled_uv_job(
        ...
        *,
        ...
        Example:
            ...
            ```python
            >>> from megatensors._hub import create_scheduled_uv_job
            >>> script = "https://raw.githubusercontent.com/huggingface/trl/refs/heads/main/trl/scripts/sft.py"
            >>> script_args = ["--model_name_or_path", "Qwen/Qwen2-0.5B", "--dataset_name", "trl-lib/Capybara", "--push_to_hub"]
    ...
    def _create_uv_command_env_and_secrets(
        ...
        *,
        ...
        if constants.MEGA_JOBS_ARTIFACTS_MOUNT_PATH in existing_mount_paths:
            raise ValueError(
                f"Mount path {constants.MEGA_JOBS_ARTIFACTS_MOUNT_PATH!r} is reserved for Jobs artifacts when running local scripts. Mount your volume at a different path."
            )
        ...
        mount_path = constants.MEGA_JOBS_ARTIFACTS_MOUNT_PATH
    ...
    def _upload_scripts_to_bucket(
        ...
        *,
        ...
        """
        bucket_id = f"{namespace}/{constants.MEGA_JOBS_ARTIFACTS_BUCKET_NAME}"
    ...
    def sync_job_volume(
        ...
        *,
        ...
        Example:
            ```python
            >>> from megatensors._hub import run_uv_job, sync_job_volume
        ...
        self.sync_bucket(str(source_path), f"mega://buckets/{bucket_id}/{folder}", token=token)
    ...
    def create_bucket(
        ...
        *,
        ...
        Args:
            ...
            resource_group_id (`str`, *optional*):
                Resource group in which to create the bucket. MEGA Resource Groups are available to entitled Team
                and Enterprise organizations and define which members can access the resource. The ID
                of a resource group can be found in the URL of the resource's page on the Hub
                (e.g. `"9a70e516-3145-4ca5-a2cb-1988c8d9e001"`). To learn more about resource groups, see
                https://mega.tensorplay.cn/docs/hub/organizations#resource-groups.
            region (`Literal["us", "eu"]`, *optional*):
                Durable storage region in which to create the bucket. Can be one of `"us"` or `"eu"`. If not specified,
                the default is `"us"`. The server accepts a region only when a writable Xet data plane is configured;
                `"eu"` also requires an entitled Team or Enterprise organization.
            exist_ok (`bool`, *optional*, defaults to `False`):
        ...
        Example:
            ```python
            >>> from megatensors._hub import create_bucket

            >>> url = create_bucket(bucket_id="my-bucket")
            >>> url.bucket_id
            'user/my-bucket'
            >>> url.url
            'https://mega.tensorplay.cn/buckets/user/my-bucket'
            >>> url.uri.to_uri()
            'mega://buckets/user/my-bucket'
    ...
    def bucket_info(
        ...
        *,
        ...
        Example:
            ```python
            >>> from megatensors._hub import bucket_info
    ...
    def list_buckets(
        ...
        *,
        ...
        Example:
            ```python
            >>> from megatensors._hub import list_buckets
        ...
        for item in paginate(
            f"{self.endpoint}/api/buckets/{namespace}", params=params, headers=self._build_mega_headers(token=token)
        ):
    ...
    def delete_bucket(
        ...
        *,
        ...
        Example:
            ```python
            >>> from megatensors._hub import delete_bucket
    ...
    def move_bucket(
        ...
        *,
        ...
        Example:
            ```python
            >>> from megatensors._hub import move_bucket
    ...
    def list_bucket_tree(
        ...
        *,
        ...
        Example:
            ```python
            >>> from megatensors._hub import list_bucket_tree
    ...
    def get_bucket_paths_info(
        ...
        *,
        ...
        Example:
        ```py
        >>> from megatensors._hub import get_bucket_paths_info
    ...
    def copy_files(self, source: str, destination: str, *, token: str | bool | None = None) -> None:
        ...
        > [!WARNING]
        > Server-side copies only work within the same [storage region](https://mega.tensorplay.cn/docs/hub/storage-regions).

        Args:
            source (`str`):
                Source location as an `mega://` URI. Can be a bucket path (e.g. `"mega://buckets/my-bucket/path/to/file"`)
                or a repo path (e.g. `"mega://username/my-model/weights.bin"`, `"mega://datasets/username/my-dataset/data/"`).
            destination (`str`):
                Destination location as an `mega://` URI pointing to a bucket (e.g. `"mega://buckets/my-bucket/target/path"`)
                or a repository (e.g. `"mega://username/my-model/target/path"`).
            token (`bool` or `str`, *optional*):
                A valid user access token (string). Defaults to the locally saved
                token, which is the recommended method for authentication (see
                https://mega.tensorplay.cn/docs/megatensors._hub/quick-start#authentication).
        ...
        Example:
            ```python
            >>> from megatensors._hub import copy_files

            # Copy a single file between buckets
            >>> copy_files("mega://buckets/my-bucket/data.bin", "mega://buckets/other-bucket/data.bin")

            # Copy a folder into another bucket (nests: backup/models/...)
            >>> copy_files("mega://buckets/my-bucket/models", "mega://buckets/other-bucket/backup/")

            # Copy folder contents (trailing /): files go directly into backup/
            >>> copy_files("mega://buckets/my-bucket/models/", "mega://buckets/other-bucket/backup/")

            # Copy a file from a model repo to a bucket
            >>> copy_files("mega://username/my-model/model.safetensors", "mega://buckets/my-bucket/")

            # Copy an entire dataset to a bucket
            >>> copy_files("mega://datasets/username/my-dataset/", "mega://buckets/my-bucket/datasets/")

            # Copy files between repositories
            >>> copy_files("mega://username/source-model/", "mega://username/dest-model/")

            # Copy a file from one repo to another
            >>> copy_files("mega://username/source-model/config.json", "mega://username/dest-model/config.json")
            ```
        """
        source_uri = parse_mega_uri(source)
        destination_uri = parse_mega_uri(destination)
        ...
        *,
        ...
        if pending_downloads:
            ...
            def _download_and_collect(item: tuple[str, str]) -> None:
                ...
                local_path = self.mega_hub_download(
                    repo_id=source.id,
    ...
    def batch_bucket_files(
        ...
        *,
        ...
        Example:
            ```python
            >>> from megatensors._hub import batch_bucket_files
    ...
    def get_bucket_file_metadata(
        ...
        *,
        ...
        Example:
            ```python
            >>> from megatensors._hub import get_bucket_file_metadata
    ...
    def download_bucket_files(
        ...
        *,
        ...
        Example:
            ```python
            >>> from megatensors._hub import download_bucket_files
        ...
        with XetDownloadProgressReporter(
            ...
            name="megatensors._hub.download_bucket_files",
        ) as progress:
            try:
    ...
    def sync_bucket(
        ...
        *,
        ...
        This is equivalent to the ``mega buckets sync`` CLI command. One of ``source`` or ``dest`` must be a bucket path
        (``mega://buckets/...``) and the other must be a local directory path.

        Args:
            source (`str`, *optional*):
                Source path: local directory or ``mega://buckets/namespace/bucket_name(/prefix)``.
                Required unless using ``apply``.
            dest (`str`, *optional*):
                Destination path: local directory or ``mega://buckets/namespace/bucket_name(/prefix)``.
                Required unless using ``apply``.
        ...
        Example:
            ...
            >>> api.sync_bucket("./data", "mega://buckets/username/my-bucket")

            # Download bucket to local directory
            >>> api.sync_bucket("mega://buckets/username/my-bucket", "./data")

            # Sync with delete and filtering
            >>> api.sync_bucket(
            ...     "./data",
            ...     "mega://buckets/username/my-bucket",
            ...     delete=True,
            ...     include=["*.safetensors"],
            ... )

            # Dry run: preview what would be synced
            >>> plan = api.sync_bucket("./data", "mega://buckets/username/my-bucket", dry_run=True)
            >>> plan.summary()
            {'uploads': 3, 'downloads': 0, 'deletes': 0, 'skips': 1, 'total_size': 4096}

            # Save plan for review, then apply
            >>> api.sync_bucket("./data", "mega://buckets/username/my-bucket", plan="sync-plan.jsonl")
...
def _parse_revision_from_pr_url(pr_url: str) -> str:
    ...
    Example:
    ```py
    >>> _parse_revision_from_pr_url("https://mega.tensorplay.cn/bigscience/bloom/discussions/2")
    "refs/pr/2"
    ```
    """
...
# ``HfApi`` and this parser name are source-compatibility aliases only. Both
# operate against the configured MEGA Hub endpoint and retain their canonical
# ``Mega*`` names for new code.
HfApi = MegaApi
repo_type_and_id_from_hf_id = repo_type_and_id_from_mega_id
