...
from .file_download import mega_hub_url, http_get
from .mega_api import MegaApi, SPECIAL_REFS_REVISION_REGEX, BucketFile, BucketFolder, LastCommitInfo, RepoFile, RepoFolder
from .utils import MegaValidationError, mega_raise_for_status, http_backoff, http_stream_backoff, parse_mega_uri
from .utils.insecure_hashlib import md5


@dataclass
class MegaFileSystemResolvedPath:
    """Top level Data structure containing information about a resolved MEGA file system path."""
...
@dataclass
class MegaFileSystemResolvedRepositoryPath(MegaFileSystemResolvedPath):
...
@dataclass
class MegaFileSystemResolvedBucketPath(MegaFileSystemResolvedPath):
...
class _Cached(_cached_base):
    """
    Metaclass for caching MegaFileSystem instances according to the args.
    ...
    def __call__(cls, *args, **kwargs):
        ...
        # MegaFileSystem picks up defaults the same way other fsspec filesystems do.
...
class MegaFileSystem(fsspec.AbstractFileSystem, metaclass=_Cached):  # ty: ignore[conflicting-metaclass]
    """
    Access a remote MEGA Hub repository as if it were a local file system.

    > [!WARNING]
    > [`MegaFileSystem`] provides fsspec compatibility, which is useful for libraries that require it (e.g., reading
    >     repository data directly with `pandas`). However, it introduces additional overhead due to the filesystem
    >     layer. For better performance and reliability, it's recommended to use `MegaApi` methods when possible.

    The file system supports paths for the `mega://` protocol, using these URL schemes:

    * Models, Datasets and Spaces repositories:

        ```
        mega://<repo-id>[@<revision>]/<path/in/repo>
        mega://datasets/<repo-id>[@<revision>]/<path/in/repo>
        mega://spaces/<repo-id>[@<revision>]/<path/in/repo>
        ```
    ...
    * Buckets (generic storage):
    ...
    When using [`MegaFileSystem`] directly, the `mega://` protocol prefix is optional.

    Args:
        endpoint (`str`, *optional*):
                Endpoint of the MEGA Hub. Defaults to the configured endpoint.
    ...
    ```python
    >>> from megatensors._hub import hffs
    ...
    >>> hffs = MegaFileSystem(token=token)
    ```
    """
    ...
    def __init__(
        ...
        **storage_options,
        ...
        self.endpoint = self._api.endpoint
    ...
    def _repo_and_revision_exist(
        ...
        if (repo_type, repo_id, revision) not in self._repo_and_revision_exists_cache:
            try:
                self._api.repo_info(
                    repo_id, revision=revision, repo_type=repo_type, timeout=constants.MEGA_HUB_ETAG_TIMEOUT
                )
            except (RepositoryNotFoundError, MegaValidationError) as e:
    ...
    def resolve_path(
    ...
    ) -> MegaFileSystemResolvedRepositoryPath | MegaFileSystemResolvedBucketPath:
        """
        Resolve a MEGA file system path into its components.
        ...
        Returns:
            [`MegaFileSystemResolvedPath`]: Resolved path information containing `repo_type`, `repo_id`, `revision` and `path_in_repo`.
        ...
        parsed = parse_mega_uri(f"{constants.MEGA_PROTOCOL}{path}")
        ...
        if parsed.is_bucket:
            ...
            return MegaFileSystemResolvedBucketPath(bucket_id=parsed.id, path=parsed.path_in_repo)
        ...
        if revision is not None and parsed.revision is not None and parsed.revision != revision:
            # The caller provided an explicit revision that conflicts with what parse_mega_uri
            # parsed. This can happen when a user has a branch literally named "refs" and a
            # file at "pr/10" — parse_mega_uri would greedily match "refs/pr/10" as a special
            # ref. Fall back to simple '@' splitting so the caller's revision wins.
            path_without_type = path.split("/", 1)[1] if path.split("/")[0] in constants.MEGA_URI_TYPE_PREFIXES else path
            ...
            if not repo_and_revision_exist:
                _raise_file_not_found(path, err)
            return MegaFileSystemResolvedRepositoryPath(
    ...
    def invalidate_cache(self, path: str | None = None) -> None:
        ...
        if not path:
            ...
            if not resolved_path.path:
                if isinstance(resolved_path, MegaFileSystemResolvedRepositoryPath):
    ...
    def _open(  # type: ignore
        ...
        **kwargs,
    ) -> Union["MegaFileSystemFile", "MegaFileSystemStreamFile"]:
        ...
        if block_size == 0:
            return MegaFileSystemStreamFile(self, path, mode=mode, revision=revision, **kwargs)
        else:
            return MegaFileSystemFile(self, path, mode=mode, revision=revision, **kwargs)

    def _rm(self, path: str, revision: str | None = None, **kwargs) -> None:
        resolved_path = self.resolve_path(path, revision=revision)
        if isinstance(resolved_path, MegaFileSystemResolvedBucketPath):
    ...
    def rm(
        ...
        **kwargs,
        ...
        > [!WARNING]
        > Note: When possible, use `MegaApi.delete_file()` for better performance.
    ...
    def ls(
        ...
        > [!WARNING]
        > Note: When possible, use `MegaApi.list_repo_tree()` for better performance.
    ...
    def _ls_tree(
        ...
        if path in self.dircache and not refresh:
            ...
            dirs_not_expanded = []
            if expand_info and isinstance(resolved_path, MegaFileSystemResolvedRepositoryPath):
    ...
    def _list_bucket_tree_with_folders(
        ...
        """Same as `MegaApi.list_bucket_tree` but always includes folders"""
    ...
    def cp_file(self, path1: str, path2: str, revision: str | None = None, **kwargs) -> None:
        ...
        > [!WARNING]
        > Note: When possible, use `MegaApi.upload_file()` for better performance.
        ...
        if isinstance(resolved_path1, MegaFileSystemResolvedBucketPath) or isinstance(
            resolved_path2, MegaFileSystemResolvedBucketPath
        ):
    ...
    def info(self, path: str, refresh: bool = False, revision: str | None = None, **kwargs) -> dict[str, Any]:
        ...
        > [!WARNING]
        > Note: When possible, use `MegaApi.get_paths_info()` or `MegaApi.repo_info()`  for better performance
        > (or `MegaApi.get_bucket_paths_info()` or `MegaApi.bucket_info()` for buckets)
        ...
        if not resolved_path.path:
            ...
            if isinstance(resolved_path, MegaFileSystemResolvedRepositoryPath) and expand_info:
        ...
        elif isinstance(resolved_path, MegaFileSystemResolvedBucketPath):
            ...
            if refresh or out is None or (expand_info and out and out["last_commit"] is None):
                ...
                root_path = MegaFileSystemResolvedRepositoryPath(
    ...
    def exists(self, path, **kwargs):
        ...
        > [!WARNING]
        > Note: When possible, use `MegaApi.file_exists()` for better performance.
    ...
    def get_file(self, rpath, lpath, callback=_DEFAULT_CALLBACK, outfile=None, **kwargs) -> None:
        ...
        > [!WARNING]
        > Note: When possible, use `MegaApi.mega_hub_download()` or `MegaApi.download_bucket_files` for better performance.
        ...
        try:
            http_get(
                ...
                resume_size=0,
                headers=self._api._build_mega_headers(),
    ...
    def transaction(self):
        ...
        # See https://github.com/huggingface/megatensors._hub/issues/1733
    ...
    def __reduce__(self):
        # re-populate the instance cache at MegaFileSystem._cache and re-populate the state of every instance
...
class MegaFileSystemFile(fsspec.spec.AbstractBufferedFile):
    def __init__(self, fs: MegaFileSystem, path: str, revision: str | None = None, **kwargs):
        try:
    ...
    def _fetch_range(self, start: int, end: int) -> bytes:
        headers = {
            ...
            **self.fs._api._build_mega_headers(),
        }
        url = self.url()
        r = http_backoff("GET", url, headers=headers, timeout=constants.MEGA_HUB_DOWNLOAD_TIMEOUT)
    ...
    def _upload_chunk(self, final: bool = False) -> None:
        ...
        if final:
            ...
            if isinstance(self.resolved_path, MegaFileSystemResolvedBucketPath):
...
class MegaFileSystemStreamFile(fsspec.spec.AbstractBufferedFile):
    def __init__(
        self,
        fs: MegaFileSystem,
        path: str,
        ...
        **kwargs,
        ...
        if block_size != 0:
            raise ValueError(f"MegaFileSystemStreamFile only supports block_size=0 but got {block_size}")
        if cache_type != "none":
            raise ValueError(f"MegaFileSystemStreamFile only supports cache_type='none' but got {cache_type}")
        if "w" in mode:
            raise ValueError(f"MegaFileSystemStreamFile only supports reading but got mode='{mode}'")
        try:
            self.resolved_path = fs.resolve_path(path, revision=revision)
        except FileNotFoundError as e:
            if "w" in kwargs.get("mode", ""):
    ...
    def _open_connection(self):
        ...
        url = self.url()
        headers = self.fs._api._build_mega_headers()
        if self.loc > 0:
        ...
        try:
            mega_raise_for_status(self.response)
...
def _raise_file_not_found(path: str, err: Exception | None) -> NoReturn:
    ...
    elif isinstance(err, MegaValidationError):
        msg = f"{path} (invalid repository id)"
    raise FileNotFoundError(msg) from err


def reopen(fs: MegaFileSystem, path: str, mode: str, block_size: int, cache_type: str):
...
# Import-compatible aliases for fsspec users migrating Hub code to MEGA.
HfFileSystem = MegaFileSystem
HfFileSystemFile = MegaFileSystemFile
HfFileSystemResolvedPath = MegaFileSystemResolvedPath
HfFileSystemStreamFile = MegaFileSystemStreamFile
