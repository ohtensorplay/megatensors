...
from megatensors._hub.mega_api import upload_file
from megatensors._hub.repocard_data import (
    CardData,
    DatasetCardData,
    EvalResult,
    ModelCardData,
...
from megatensors._hub.utils import MegaHubHTTPError, get_session, mega_raise_for_status, is_jinja_available, yaml_dump

from . import constants
from .errors import EntryNotFoundError
from .utils import SoftTemporaryDirectory, logging, validate_mega_hub_args
...
class RepoCard:
    ...
    def __init__(self, content: str, ignore_metadata_errors: bool = False):
        ...
        Example:
            ```python
            >>> from megatensors._hub.repocard import RepoCard
            >>> text = '''
            ... ---
            ... language: en
            ... license: mit
    ...
    def load(
        ...
        """Initialize a RepoCard from a MEGA Hub repo's README.md or a local filepath.

        Args:
            repo_id_or_path (`Union[str, Path]`):
                The repo ID associated with a MEGA Hub repo or a local filepath.
            repo_type (`str`, *optional*):
                The type of MEGA repo to push to. Defaults to None, which will use "model". Other options
            ...
            token (`str`, *optional*):
                Authentication token, obtained with `megatensors._hub.MegaApi.login` method. Will default to the stored token.
            ignore_metadata_errors (`str`):
                If True, errors while parsing the metadata section will be ignored. Some information might be lost during
                the process. Use it at your own risk.

        Returns:
            [`megatensors._hub.repocard.RepoCard`]: The RepoCard (or subclass) initialized from the repo's
    ...
    def validate(self, repo_type: str | None = None):
        """Validates card against MEGA Hub's card validation logic.
        Using this function requires access to the internet, so it is only called
        internally by [`megatensors._hub.repocard.RepoCard.push_to_hub`].

        Args:
            repo_type (`str`, *optional*, defaults to "model"):
                The type of MEGA repo to push to. Options are "model", "dataset", and "space".
        ...
        try:
            response = get_session().post("https://mega.tensorplay.cn/api/validate-yaml", json=body, headers=headers)
            mega_raise_for_status(response)
        except MegaHubHTTPError as exc:
    ...
    def push_to_hub(
        ...
        """Push a RepoCard to a MEGA Hub repo.

        Args:
            repo_id (`str`):
                The repo ID of the MEGA Hub repo to push to. Example: "nateraw/food".
            token (`str`, *optional*):
                Authentication token, obtained with `megatensors._hub.MegaApi.login` method. Will default to
                the stored token.
            repo_type (`str`, *optional*, defaults to "model"):
                The type of MEGA repo to push to. Options are "model", "dataset", and "space". If this
    ...
    def from_template(
        ...
        **template_kwargs,
        ...
        Args:
            card_data (`megatensors._hub.CardData`):
                A megatensors._hub.CardData instance containing the metadata you want to include in the YAML
                header of the repo card on the MEGA Hub.
        ...
        Returns:
            [`megatensors._hub.repocard.RepoCard`]: A RepoCard instance with the specified card data and content from the
            template.
        """
...
class ModelCard(RepoCard):
    ...
    def from_template(  # type: ignore # violates Liskov property but easier to use
        ...
        **template_kwargs,
        ...
        https://github.com/huggingface/megatensors._hub/blob/main/src/megatensors._hub/templates/modelcard_template.md

        Templates are Jinja2 templates that can be customized by passing keyword arguments.

        Args:
            card_data (`megatensors._hub.ModelCardData`):
                A megatensors._hub.ModelCardData instance containing the metadata you want to include in the YAML
                header of the model card on the MEGA Hub.
            template_path (`str`, *optional*):
                A path to a markdown file with optional Jinja template variables that can be filled
                in with `template_kwargs`. Defaults to the default template.
            template_str (`str`, *optional*):
        ...
        Returns:
            [`megatensors._hub.ModelCard`]: A ModelCard instance with the specified card data and content from the
            template.

        Example:
            ```python
            >>> from megatensors._hub import ModelCard, ModelCardData, EvalResult
            ...
            >>> card = ModelCard.from_template(
            ...     card_data=card_data,
            ...     template_path='./src/megatensors._hub/templates/modelcard_template.md',
...
class DatasetCard(RepoCard):
    ...
    def from_template(  # type: ignore # violates Liskov property but easier to use
        ...
        **template_kwargs,
        ...
        https://github.com/huggingface/megatensors._hub/blob/main/src/megatensors._hub/templates/datasetcard_template.md

        Templates are Jinja2 templates that can be customized by passing keyword arguments.

        Args:
            card_data (`megatensors._hub.DatasetCardData`):
                A megatensors._hub.DatasetCardData instance containing the metadata you want to include in the YAML
                header of the dataset card on the MEGA Hub.
            template_path (`str`, *optional*):
                A path to a markdown file with optional Jinja template variables that can be filled
                in with `template_kwargs`. Defaults to the default template.
            template_str (`str`, *optional*):
        ...
        Returns:
            [`megatensors._hub.DatasetCard`]: A DatasetCard instance with the specified card data and content from the
            template.

        Example:
            ```python
            >>> from megatensors._hub import DatasetCard, DatasetCardData
            ...
            >>> card = DatasetCard.from_template(
            ...     card_data=card_data,
            ...     template_path='./src/megatensors._hub/templates/datasetcard_template.md',
            ...     custom_template_var='custom value',  # will be replaced in template if it exists
            ... )
...
def metadata_eval_result(
    *,
    ...
    Args:
        ...
        metrics_id (`str`):
            Example: wer. A metric id from https://mega.tensorplay.cn/metrics.
        metrics_value (`Any`):
            The value from the metric. Example: 20.0 or "20.0 ± 1.2".
        dataset_pretty_name (`str`):
            The name of the dataset in natural language.
        dataset_id (`str`):
            Example: common_voice. A dataset id from https://mega.tensorplay.cn/datasets.
        metrics_config (`str`, *optional*):
            The name of the metric configuration used in `load_metric()`.
            Example: bleurt-large-512 in `load_metric("bleurt", "bleurt-large-512")`.
        metrics_verified (`bool`, *optional*, defaults to `False`):
            Indicates whether the metrics originate from MEGA's [evaluation service](https://mega.tensorplay.cn/spaces/autoevaluate/model-evaluator) or not. Automatically computed by MEGA, do not set.
        ...
        metrics_verification_token (`bool`, *optional*):
            A JSON Web Token that is used to verify whether the metrics originate from MEGA's [evaluation service](https://mega.tensorplay.cn/spaces/autoevaluate/model-evaluator) or not.
    ...
    Example:
        ```python
        >>> from megatensors._hub import metadata_eval_result
...
def metadata_update(
    ...
    *,
    ...
    """
    Updates the metadata in the README.md of a repository on the MEGA Hub.
    ...
    Args:
        ...
        token (`str`, *optional*):
            The MEGA authentication token.
        commit_message (`str`, *optional*):
            The summary / title / first line of the generated commit. Defaults to
            `f"Update metadata with megatensors._hub"`
        commit_description (`str` *optional*)
            The description of the generated commit
        revision (`str`, *optional*):
    ...
    Example:
        ```python
        >>> from megatensors._hub import metadata_update
        ...
        ```
    """
    commit_message = commit_message if commit_message is not None else "Update metadata with megatensors._hub"
