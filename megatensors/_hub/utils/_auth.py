...
def get_token() -> str | None:
    ...
    Note: in most cases, you should use [`megatensors._hub.utils.build_mega_headers`] instead. This method is only useful
          if you want to retrieve the token for other purposes than sending an HTTP request.

    The token is retrieved from the `MEGA_TOKEN` environment variable, then from the MEGA active token file.
    Returns None if user is not logged in. To log in, use [`login`] or `mega auth login`.
    ...
    Note: if `MEGA_OIDC_RESOURCE` is set but the OIDC token exchange fails, this raises instead of returning `None`,
...
def _get_token_from_google_colab() -> str | None:
    ...
    # => use a lock
    # See https://github.com/huggingface/megatensors._hub/issues/1952 for more details.
    with _GOOGLE_COLAB_SECRET_LOCK:
        ...
        try:
            token = userdata.get("MEGA_TOKEN")
            _GOOGLE_COLAB_SECRET = _clean_token(token)
        except userdata.NotebookAccessError:
            # Means the user has a secret called `MEGA_TOKEN` and refused notebook access to it.
            # => warn user but ignore error => do not re-request access to user
            warnings.warn(
                "\nAccess to the secret `MEGA_TOKEN` has not been granted on this notebook."
                "\nYou will not be requested again."
                "\nPlease restart the session if you want to be prompted again."
            )
            _GOOGLE_COLAB_SECRET = None
        except userdata.SecretNotFoundError:
            # No `MEGA_TOKEN` secret defined: simply not logged in via the Colab vault. Not worth a
            ...
            logger.info(
                "The secret `MEGA_TOKEN` does not exist in your Colab secrets. Run `megatensors._hub.login()` to"
            ...
            warnings.warn(f"\nError while fetching `MEGA_TOKEN` secret value from your vault: '{str(e)}'.")
            _GOOGLE_COLAB_SECRET = None
...
def _get_token_from_environment() -> str | None:
    return _clean_token(os.environ.get("MEGA_TOKEN"))


def _get_token_from_file() -> str | None:
    try:
        return _clean_token(Path(constants.MEGA_TOKEN_PATH).read_text())
...
def _get_token_from_oidc() -> str | None:
    ...
    See `megatensors._hub._oidc` for the MEGA trusted-publisher flow.
    """
    resource = os.environ.get("MEGA_OIDC_RESOURCE")
    if not resource:
        return None
    ...
    with _OIDC_TOKEN_LOCK:
        ...
        subject_token = os.environ.get("MEGA_OIDC_ID_TOKEN") or None
        if subject_token is None and detect_provider() is None:
            raise OIDCError(
                "MEGA_OIDC_RESOURCE is set but no OIDC id token is available: not running in a supported "
                "CI provider (github) and MEGA_OIDC_ID_TOKEN is not set. Set MEGA_OIDC_ID_TOKEN to the id "
                "token minted by your CI provider, or unset MEGA_OIDC_RESOURCE."
            )

        result = oidc_login(resource=resource, subject_token=subject_token)
        token = result["access_token"]
        expires_in = int(result.get("expires_in", 3600))
        # A pre-supplied MEGA_OIDC_ID_TOKEN can't be re-minted, so refreshing early is pointless (the id
...
class _OAuthRefreshCache(TypedDict):
    file_token: str  # token as read from the active token file (cache key)
...
_OAUTH_REFRESH_MARGIN = 30 * 60  # refresh shortly before the 8-hour device access token expires
...
def _get_token_from_file_refreshed() -> str | None:
    """Get the token from the active token file, transparently refreshing it if close to expiry."""
...
def _refresh_oauth_token_if_needed(token: str) -> str:
    ...
    with _OAUTH_REFRESH_LOCK:
        ...
        token_name, fields = next(
            ((name, fields) for name, fields in _read_stored_tokens_full().items() if fields.get("mega_token") == token),
            (None, {}),
        )
            ...
            with WeakFileLock(constants.MEGA_STORED_TOKENS_PATH + ".lock", timeout=30):
                ...
                if fields.get("mega_token") != token:
                    ...
                    if _get_token_from_file() == token:
                        _write_secret(Path(constants.MEGA_TOKEN_PATH), new_token)
                    logger.debug("The access token was refreshed.")
        except Exception as error:
            logger.debug("Could not refresh the access token.", exc_info=True)
            if isinstance(error, DeviceCodeError) and error.error_code == OAuthErrorCode.INVALID_GRANT:
                ...
                logger.warning(
                    "Your MEGA access token has expired and could not be refreshed "
                    "(session expired or revoked). Run `mega auth login --force` to sign in again."
                )
                recheck_at = float("inf")
            else:
                # Transient failure (offline, server error, ...): retry later.
                _warn_refresh_failure_once("Could not refresh your MEGA access token. Will retry later.")
                recheck_at = now + _OAUTH_RECHECK_INTERVAL
...
def get_stored_tokens() -> dict[str, str]:
    ...
    The file is located at the MEGA stored tokens path, defaulting to `~/.cache/mega/stored_tokens`.
    If the file does not exist, an empty dictionary is returned.

    Returns: `dict[str, str]`
        Key is the token name and value is the token.
    """
    return {token_name: fields.get("mega_token", "") for token_name, fields in _read_stored_tokens_full().items()}


def _read_stored_tokens_full() -> dict[str, dict[str, str]]:
    """Read all sections of the stored tokens INI file, with all their fields.

    Beside `mega_token`, sections for OAuth tokens also carry `refresh_token` and `expires_at`
    (unix timestamp), used by [`get_token`] to transparently refresh them.
    """
    tokens_path = Path(constants.MEGA_STORED_TOKENS_PATH)
    ...
    try:
        ...
        logger.debug("Could not parse the locally stored access tokens.", exc_info=True)
        logger.error("Could not read locally stored access tokens.")
        return {}
...
def _save_stored_tokens_full(stored_tokens: dict[str, dict[str, str]]) -> None:
    ...
    buf = io.StringIO()
    config.write(buf)
    _write_secret(Path(constants.MEGA_STORED_TOKENS_PATH), buf.getvalue())
...
def _save_token(
    ...
    logger.debug("The token was added to the local credential store.")
