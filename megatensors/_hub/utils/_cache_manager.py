...
"""Contains utilities to manage the MEGA cache directory."""
...
from megatensors._hub.errors import CacheNotFound, CorruptedCacheException

from ..constants import MEGA_HUB_CACHE
from . import logging
...
class DeleteCacheStrategy:
    ...
    [`~utils.MegaCacheInfo.delete_revisions`]. See documentation for usage example.
...
class MegaCacheInfo:
    ...
    def delete_revisions(self, *revisions: str) -> DeleteCacheStrategy:
        ...
        Examples:
        ```py
        >>> from megatensors._hub import scan_cache_dir
    ...
    def export_as_table(self, *, verbosity: int = 0) -> str:
        """Generate a table from the [`MegaCacheInfo`] object.
        ...
        Example:
        ```py
        >>> from megatensors._hub.utils import scan_cache_dir

        >>> mega_cache_info = scan_cache_dir()
        MegaCacheInfo(...)

        >>> print(mega_cache_info.export_as_table())
        REPO ID                                             REPO TYPE SIZE ON DISK NB FILES LAST_ACCESSED LAST_MODIFIED REFS LOCAL PATH
        --------------------------------------------------- --------- ------------ -------- ------------- ------------- ---- --------------------------------------------------------------------------------------------------
        roberta-base                                        model             2.7M        5 1 day ago     1 week ago    main ~/.cache/mega/hub/models--roberta-base
        suno/bark                                           model             8.8K        1 1 week ago    1 week ago    main ~/.cache/mega/hub/models--suno--bark
        t5-base                                             model           893.8M        4 4 days ago    7 months ago  main ~/.cache/mega/hub/models--t5-base
        t5-large                                            model             3.0G        4 5 weeks ago   5 months ago  main ~/.cache/mega/hub/models--t5-large

        >>> print(mega_cache_info.export_as_table(verbosity=1))
        REPO ID                                             REPO TYPE REVISION                                 SIZE ON DISK NB FILES LAST_MODIFIED REFS LOCAL PATH
        --------------------------------------------------- --------- ---------------------------------------- ------------ -------- ------------- ---- -----------------------------------------------------------------------------------------------------------------------------------------------------
        roberta-base                                        model     e2da8e2f811d1448a5b465c236feacd80ffbac7b         2.7M        5 1 week ago    main ~/.cache/mega/hub/models--roberta-base/snapshots/e2da8e2f811d1448a5b465c236feacd80ffbac7b
        suno/bark                                           model     70a8a7d34168586dc5d028fa9666aceade177992         8.8K        1 1 week ago    main ~/.cache/mega/hub/models--suno--bark/snapshots/70a8a7d34168586dc5d028fa9666aceade177992
        t5-base                                             model     a9723ea7f1b39c1eae772870f3b547bf6ef7e6c1       893.8M        4 7 months ago  main ~/.cache/mega/hub/models--t5-base/snapshots/a9723ea7f1b39c1eae772870f3b547bf6ef7e6c1
        t5-large                                            model     150ebc2c4b72291e770f58e6057481c8d2ed331a         3.0G        4 5 months ago  main ~/.cache/mega/hub/models--t5-large/snapshots/150ebc2c4b72291e770f58e6057481c8d2ed331a
        ```
...
def scan_cache_dir(cache_dir: str | Path | None = None) -> MegaCacheInfo:
    """Scan the entire MEGA cache-system and return a [`~MegaCacheInfo`] structure.

    Use `scan_cache_dir` in order to programmatically scan your cache-system. The cache
    will be scanned repo by repo. If a repo is corrupted, a [`~CorruptedCacheException`]
    will be thrown internally but captured and returned in the [`~MegaCacheInfo`]
    ...
    You can also print a detailed report directly from the `mega` command line using:
    ```text
    > mega cache ls
    ...
    Args:
        cache_dir (`str` or `Path`, `optional`):
            Cache directory to cache. Defaults to the default MEGA cache directory.
    ...
    Returns: a [`~MegaCacheInfo`] object.
    """
    if cache_dir is None:
        cache_dir = MEGA_HUB_CACHE

    cache_dir = Path(cache_dir).expanduser().resolve()
    if not cache_dir.exists():
        raise CacheNotFound(
            f"Cache directory not found: {cache_dir}. Please use `cache_dir` argument or set `MEGA_HUB_CACHE` environment variable.",
            cache_dir=cache_dir,
        )

    if cache_dir.is_file():
        raise ValueError(
            f"Scan cache expects a directory but found a file: {cache_dir}. Please use `cache_dir` argument or set `MEGA_HUB_CACHE` environment variable."
        )
...
def _scan_incomplete_files(cache_dir: Path) -> frozenset[CachedIncompleteFileInfo]:
    ...
    [`~MegaCacheInfo`] returned by [`scan_cache_dir`].
    """
...
def _scan_cached_repo(repo_path: Path) -> CachedRepoInfo:
    ...
    if "--" not in repo_path.name:
        raise CorruptedCacheException(f"Repo path is not a valid MEGA cache directory: {repo_path}")
...
# Compatibility spelling for callers porting cache inspection code.
HFCacheInfo = MegaCacheInfo
