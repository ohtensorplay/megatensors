...
"""Contains utilities to flag a feature as "experimental" in MEGA Hub."""
...
def experimental(fn: Callable) -> Callable:
    ...
    Warnings can be disabled by setting `MEGA_HUB_DISABLE_EXPERIMENTAL_WARNING=1` as environment variable.
    ...
    ```python
    >>> from megatensors._hub.utils import experimental
    ...
    notice. You can disable this warning by setting `MEGA_HUB_DISABLE_EXPERIMENTAL_WARNING=1` as environment variable.
    Hello world!
    ```
    """
    ...
    def _inner_fn(*args, **kwargs):
        if not constants.MEGA_HUB_DISABLE_EXPERIMENTAL_WARNING:
            warnings.warn(
                f"'{name}' is experimental and might be subject to breaking changes in the future without prior notice."
                " You can disable this warning by setting `MEGA_HUB_DISABLE_EXPERIMENTAL_WARNING=1` as environment"
                " variable.",
                UserWarning,
            )
