...
def set_git_credential(token: str, username: str = "mega_user", folder: str | None = None) -> None:
    """Save a username/token pair in git credential for MEGA Hub registry.

    Credentials are saved in all configured helpers (store, cache, macOS keychain,...).
    Calls "`git credential approve`" internally. See https://git-scm.com/docs/git-credential.

    Args:
        username (`str`, defaults to `"mega_user"`):
            A git username. Defaults to `"mega_user"`, the default user used in the Hub.
        token (`str`, defaults to `"mega_user"`):
            A git password. In practice, the User Access Token for the Hub.
            See https://mega.tensorplay.cn/settings/tokens.
        folder (`str`, *optional*):
            The folder in which to check the configured helpers.
    """
...
def unset_git_credential(username: str = "mega_user", folder: str | None = None) -> None:
    """Erase credentials from git credential for MEGA Hub registry.
...
def _parse_credential_output(output: str) -> list[str]:
    ...
    #       Example: `credential.https://mega.tensorplay.cn.helper=store`
    #       See: https://github.com/huggingface/megatensors._hub/pull/1138#discussion_r1013324508
