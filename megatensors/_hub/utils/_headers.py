...
"""Contains utilities to handle headers to send in calls to MEGA Hub."""

from megatensors._hub.errors import LocalTokenNotFoundError
...
from ._validators import validate_mega_hub_args


@validate_mega_hub_args
def build_mega_headers(
    *,
    ...
    """
    Build headers dictionary to send in a MEGA Hub call.
    ...
    token to the Hub, set `token=False` or set the `MEGA_HUB_DISABLE_IMPLICIT_TOKEN`
    ...
    the installed packages (versions of python, megatensors._hub, torch).

    Args:
        token (`str`, `bool`, *optional*):
            The token to be sent in authorization header for the Hub call:
                - if a string, it is used as the MEGA token
                ...
                - if `None`, the token is read from the machine only except if
                  `MEGA_HUB_DISABLE_IMPLICIT_TOKEN` env variable is set.
    ...
    Example:
    ```py
        >>> build_mega_headers(token="mega_***") # explicit token
        {"authorization": "Bearer mega_***", "user-agent": ""}

        >>> build_mega_headers(token=True) # explicitly use cached token
        {"authorization": "Bearer mega_***",...}

        >>> build_mega_headers(token=False) # explicitly don't use cached token
        {"user-agent": ...}

        >>> build_mega_headers() # implicit use of the cached token
        {"authorization": "Bearer mega_***",...}

        # MEGA_HUB_DISABLE_IMPLICIT_TOKEN=True # to set as env variable
        >>> build_mega_headers() # token is not sent
        {"user-agent": ...}

        >>> build_mega_headers(library_name="transformers", library_version="1.2.3")
        {"authorization": ..., "user-agent": "transformers/1.2.3; mega_hub/0.10.2; python/3.10.4; tensorflow/1.55"}
    ```
    ...
    if token_to_send is not None:
        mega_headers["authorization"] = f"Bearer {token_to_send}"
    if headers is not None:
        mega_headers.update(headers)
    return mega_headers
...
def get_token_to_send(token: bool | str | None) -> str | None:
    ...
    if token is True:
        ...
        if cached_token is None:
            raise LocalTokenNotFoundError(
                ...
                " need to provide a token or be logged in to MEGA with"
                " `mega auth login` or `megatensors._hub.login`. See"
    ...
    if constants.MEGA_HUB_DISABLE_IMPLICIT_TOKEN:
        return None
...
def _http_user_agent(
    *,
    ...
    if library_name is not None:
        ...
        ua = "unknown/None"
    ua += f"; mega_hub/{get_mega_hub_version()}"
    ua += f"; python/{get_python_version()}"

    if not constants.MEGA_HUB_DISABLE_TELEMETRY:
    ...
    origin = constants.MEGA_HUB_USER_AGENT_ORIGIN
