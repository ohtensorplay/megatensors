...
"""Contains utilities to handle HTTP requests in megatensors._hub."""
...
from megatensors._hub.errors import OfflineModeIsEnabled
...
def parse_ratelimit_headers(headers: Mapping[str, str]) -> RateLimitInfo | None:
    ...
    Example:
    ```python
    >>> from megatensors._hub.utils import parse_ratelimit_headers
    >>> headers = {
...
def _parse_repo_info_from_url(url: str) -> tuple[str | None, str | None]:
    ...
    Examples:
        ...
        ("model", "user/repo")
        >>> _parse_repo_info_from_url("https://mega.tensorplay.cn/api/datasets/user/repo/resolve/main/data.csv")
        ("dataset", "user/repo")
...
def mega_request_event_hook(request: httpx.Request) -> None:
    """
    Event hook that will be used to make HTTP requests to the MEGA Hub.
    ...
    if constants.is_offline_mode():
        raise OfflineModeIsEnabled(
            f"Cannot reach {request.url}: offline mode is enabled. To disable it, please unset the `MEGA_HUB_OFFLINE` environment variable."
        )
...
async def async_mega_request_event_hook(request: httpx.Request) -> None:
    """
    Async version of `mega_request_event_hook`.
    """
    return mega_request_event_hook(request)


async def async_mega_response_event_hook(response: httpx.Response) -> None:
...
def default_client_factory() -> httpx.Client:
    ...
    """
    return httpx.Client(
        event_hooks={"request": [mega_request_event_hook]},
...
def default_async_client_factory() -> httpx.AsyncClient:
    ...
    return httpx.AsyncClient(
        event_hooks={"request": [async_mega_request_event_hook], "response": [async_mega_response_event_hook]},
        follow_redirects=True,
        timeout=None,
    )
...
def set_client_factory(client_factory: CLIENT_FACTORY_T) -> None:
    """
    Set the HTTP client factory to be used by `megatensors._hub`.

    The client factory is a method that returns a `httpx.Client` object. On the first call to [`get_session`] the client factory
    will be used to create a new `httpx.Client` object that will be shared between all calls made by `megatensors._hub`.
...
def set_async_client_factory(async_client_factory: ASYNC_CLIENT_FACTORY_T) -> None:
    """
    Set the HTTP async client factory to be used by `megatensors._hub`.
    ...
    > [!WARNING]
    > Contrary to the `httpx.Client` that is shared between all calls made by `megatensors._hub`, the `httpx.AsyncClient` is not shared.
...
def get_session() -> httpx.Client:
    ...
    This client is shared between all calls made by `megatensors._hub`. Therefore you should not close it manually.
...
def close_session() -> None:
    """
    Close the global `httpx.Client` used by `megatensors._hub`.
        ...
        try:
            ...
            def _should_retry(response: httpx.Response) -> bool:
                ...
                if nb_tries > max_retries:
                    mega_raise_for_status(response)  # Will raise uncaught exception
...
def http_backoff(
    ...
    **kwargs,
    ...
    Example:
    ```
    >>> from megatensors._hub.utils import http_backoff
    ...
    > issue on [Github](https://github.com/huggingface/megatensors._hub).
    """
    return next(
        _http_backoff_base(
            method=method,
...
def http_stream_backoff(
    ...
    **kwargs,
    ...
    Example:
    ```
    >>> from megatensors._hub.utils import http_stream_backoff
...
def fix_mega_endpoint_in_url(url: str, endpoint: str | None) -> str:
    """Replace the default endpoint in a URL by a custom one.

    This is useful when using a proxy and MEGA Hub returns a URL with the default endpoint.
    """
    endpoint = endpoint.rstrip("/") if endpoint else constants.ENDPOINT
    # check if a proxy has been set => if yes, update the returned URL to use the proxy
    if endpoint != constants._MEGA_DEFAULT_ENDPOINT:
        url = url.replace(constants._MEGA_DEFAULT_ENDPOINT, endpoint)
    return url


def mega_raise_for_status(response: httpx.Response, endpoint_name: str | None = None) -> None:
    """
    Internal version of `response.raise_for_status()` that will refine a potential HTTPError.
    Raised exception will be an instance of [`~errors.MegaHubHTTPError`].

    This helper is meant to be the unique method to raise_for_status when making a call to the MEGA Hub.
    ...
    try:
        ...
        if error_code == "RevisionNotFound":
            ...
            message = (
                ...
                + "\nFor more details, see https://mega.tensorplay.cn/docs/megatensors._hub/authentication"
            )
            ...
            raise _format(MegaHubHTTPError, message, response) from e
        ...
        # Convert `HTTPError` into a `MegaHubHTTPError` to display request information
        # as well (request id and/or server error message)
        raise _format(MegaHubHTTPError, str(e), response) from e
...
def _warn_on_warning_headers(response: httpx.Response) -> None:
    ...
    Expected header format: 'X-Mega-Warning: topic; message'
    ...
    """
    server_warnings = response.headers.get_list("X-Mega-Warning")
...
_MegaHubHTTPErrorT = TypeVar("_MegaHubHTTPErrorT", bound=MegaHubHTTPError)


def _format(
    error_type: type[_MegaHubHTTPErrorT], custom_message: str, response: httpx.Response, **attrs: Any
) -> _MegaHubHTTPErrorT:
    server_errors = []
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                # already been read in the async event hook `async_mega_response_event_hook`.
                #
...
# Request-body fields that carry credentials; redacted from debug-log curl commands (MEGA_DEBUG).
...
# Compatibility spelling; errors are still normalized to MEGA Hub exceptions.
hf_raise_for_status = mega_raise_for_status
