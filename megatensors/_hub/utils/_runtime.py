...
def is_xet_available() -> bool:
    # Xet acceleration can be disabled explicitly through the MEGA environment.
    if constants.MEGA_HUB_DISABLE_XET:
        return False

    return is_package_available("xet")


def get_xet_version() -> str:
    return _get_version("xet")
...
def is_pydantic_available() -> bool:
    ...
    # it is installed with `pip install megatensors._hub[inference]` but it can happen when it is installed manually
    ...
    # as it is automatically imported when doing `from megatensors._hub import ...` even if the user doesn't use it.
    #
    # See https://github.com/huggingface/megatensors._hub/pull/1829 for more details.
    try:
        from pydantic import validator  # noqa: F401
    except ImportError:
        # Example: "ImportError: cannot import name 'TypeAliasType' from 'typing_extensions'"
        warnings.warn(
            "Pydantic is installed but cannot be imported. Please check your installation. `megatensors._hub` will "
...
try:
    ...
    # warning disappear. See https://github.com/huggingface/megatensors._hub/issues/1043
    #
...
# Check how megatensors._hub has been installed


def installation_method() -> Literal["brew", "mega_installer", "pip", "unknown"]:
    """Return the installation method of the current environment.

    - "mega_installer" if installed via the MEGA installer script
    - "brew" if installed via Homebrew
    - "pip" if pip is available (default fallback for standard Python environments)
    - "unknown" otherwise
    """
    # Installer detection comes first: the installer creates a venv using the
    ...
    if _is_mega_installer_installation():
        return "mega_installer"
...
def _is_brew_installation() -> bool:
    ...
    Homebrew installs the `mega` formula into a Cellar directory and creates a
    libexec virtualenv at e.g. /opt/homebrew/Cellar/mega/0.30.0/libexec/.
    We check `sys.prefix` (the venv/prefix root) for "/Cellar/mega/" rather
    ...
    return "/Cellar/mega/" in prefix


def _is_mega_installer_installation() -> bool:
    """Return ``True`` when the MEGA installer marker is present."""
    venv = sys.prefix  # points to venv root if active
    marker = Path(venv) / ".mega_installer_marker"
...
def dump_environment_info() -> dict[str, Any]:
    ...
    """
    from megatensors._hub import get_token, whoami
    from megatensors._hub.utils import is_agent, list_credential_helpers

    token = get_token()

    # Generic machine info
    info: dict[str, Any] = {
        "MEGA CLI version": get_mega_hub_version(),
    ...
    # Login info
    info["Token path ?"] = constants.MEGA_TOKEN_PATH
    ...
    info["MEGA_HUB_CACHE"] = constants.MEGA_HUB_CACHE
    info["MEGA_ASSETS_CACHE"] = constants.MEGA_ASSETS_CACHE
    info["MEGA_TOKEN_PATH"] = constants.MEGA_TOKEN_PATH
    info["MEGA_STORED_TOKENS_PATH"] = constants.MEGA_STORED_TOKENS_PATH
    info["MEGA_HUB_OFFLINE"] = constants.MEGA_HUB_OFFLINE
    info["MEGA_HUB_DISABLE_TELEMETRY"] = constants.MEGA_HUB_DISABLE_TELEMETRY
    info["MEGA_HUB_DISABLE_PROGRESS_BARS"] = constants.MEGA_HUB_DISABLE_PROGRESS_BARS
    info["MEGA_HUB_DISABLE_SYMLINKS"] = constants.MEGA_HUB_DISABLE_SYMLINKS
    info["MEGA_HUB_DISABLE_SYMLINKS_WARNING"] = constants.MEGA_HUB_DISABLE_SYMLINKS_WARNING
    info["MEGA_HUB_DISABLE_EXPERIMENTAL_WARNING"] = constants.MEGA_HUB_DISABLE_EXPERIMENTAL_WARNING
    info["MEGA_HUB_DISABLE_IMPLICIT_TOKEN"] = constants.MEGA_HUB_DISABLE_IMPLICIT_TOKEN
    info["MEGA_HUB_ETAG_TIMEOUT"] = constants.MEGA_HUB_ETAG_TIMEOUT
    info["MEGA_HUB_DOWNLOAD_TIMEOUT"] = constants.MEGA_HUB_DOWNLOAD_TIMEOUT

    print("\nCopy and paste the text below into a MEGA issue report.\n")
