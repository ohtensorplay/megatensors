...
from . import build_mega_headers, get_session, mega_raise_for_status
...
def send_telemetry(
    ...
    *,
    ...
    """
    Sends telemetry that helps track usage of different MEGA libraries.
    ...
    `MEGA_HUB_DISABLE_TELEMETRY=1` as environment variable. Telemetry is also disabled in offline mode (i.e. when setting
    `MEGA_HUB_OFFLINE=1`).
    ...
    Example:
    ```py
    >>> from megatensors._hub.utils import send_telemetry
    ...
    ```
    """
    if constants.is_offline_mode() or constants.MEGA_HUB_DISABLE_TELEMETRY:
        return
