...
class Spinner:
    """Small background spinner for synchronous terminal work.

    On a TTY the message animates in place on stderr. When stderr is redirected,
    the message is printed once so a slow operation never looks silent.
    """

    _frames = ("⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧", "⠇", "⠏")

    def __init__(self, message: str, *, enabled: bool = True, interval: float = 0.08):
    ...
    def start(self) -> None:
        ...
        if not self._active:
            ...
            return
        self._render()
        self._thread = threading.Thread(target=self._animate, name="mega-cli-spinner", daemon=True)
        self._thread.start()
    ...
    def stop(self) -> None:
        ...
        self._thread.join(timeout=max(0.2, self._interval * 2))
        self._thread = None
    ...
    def _render(self) -> None:
        sys.stderr.write(f"\r\033[K{self._frames[self._frame_index]} {self._message}")
        sys.stderr.flush()
