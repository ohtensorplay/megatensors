...
"""Contains utilities to validate argument values in `megatensors._hub`."""
...
from megatensors._hub.errors import MegaValidationError
...
def validate_mega_hub_args(fn: CallableT) -> CallableT:
    """Validate values received as argument for any public method of `megatensors._hub`.
    ...
    Example:
    ```py
    >>> from megatensors._hub.utils import validate_mega_hub_args
    ...
    megatensors._hub.utils._validators.MegaValidationError: Cannot have -- or .. in repo_id: 'other..repo..id'.
...
def validate_repo_id(repo_id: str | None) -> None:
    ...
    Example:
    ```py
    >>> from megatensors._hub.utils import validate_repo_id
    >>> validate_repo_id(repo_id="valid_repo_id")
    >>> validate_repo_id(repo_id="other..repo..id")
    megatensors._hub.utils._validators.MegaValidationError: Cannot have -- or .. in repo_id: 'other..repo..id'.
    ```

    Discussed in https://github.com/huggingface/megatensors._hub/issues/1008.
    ...
    if not isinstance(repo_id, str):
        ...
        raise MegaValidationError(f"Repo id must be a string, not {type(repo_id)}: '{repo_id}'.")
    ...
    if "--" in repo_id or ".." in repo_id:
        raise MegaValidationError(f"Cannot have -- or .. in repo_id: '{repo_id}'.")

    if repo_id.endswith(".git"):
        raise MegaValidationError(f"Repo_id cannot end by '.git': '{repo_id}'.")


def smoothly_deprecate_legacy_arguments(fn_name: str, kwargs: dict[str, Any]) -> dict[str, Any]:
    """Smoothly deprecate legacy arguments in the `megatensors._hub` codebase.
    ...
    List of deprecated arguments:
        - `proxies`:
            ...
            In megatensors._hub 0.x, `proxies` was a dictionary directly passed to `requests.request`.
            In megatensors._hub 1.x, we migrated to `httpx` which does not support `proxies` the same way.
        ...
        - `resume_download`: deprecated without replacement. `megatensors._hub` always resumes downloads whenever possible.
    ...
    if proxies is not None:
        warnings.warn(
            ...
            " environment variables or configure the `httpx.Client` manually using `megatensors._hub.set_client_factory`."
