...
from . import validate_mega_hub_args


# The authoritative MEGA CAS admits eight bounded streams. hf-xet otherwise
# starts with only two upload requests, which leaves the durable CAS-to-storage
# mirror idle during the first wave of a cold upload. Four keeps the server well
# below that admission limit while avoiding the memory cost of starting eight
# full-size XORBs at once. Respect an explicit user override.
_DEFAULT_INITIAL_UPLOAD_CONCURRENCY = "4"
_DEFAULT_MAX_TRANSFER_CONCURRENCY = "8"


def _configure_xet_upload_concurrency() -> None:
    os.environ.setdefault(
        "HF_XET_CLIENT_AC_INITIAL_UPLOAD_CONCURRENCY",
        _DEFAULT_INITIAL_UPLOAD_CONCURRENCY,
    )
    # hf-xet's adaptive controller otherwise grows to 64 streams. The MEGA
    # data plane admits eight bounded streams, and exceeding that limit turns
    # useful work into retries (including 524s on large uploads and expired
    # signed ranges on long downloads). Keep explicit user overrides intact.
    os.environ.setdefault(
        "HF_XET_CLIENT_AC_MAX_UPLOAD_CONCURRENCY",
        _DEFAULT_MAX_TRANSFER_CONCURRENCY,
    )
    os.environ.setdefault(
...
def parse_xet_file_data_from_response(response: httpx.Response, endpoint: str | None = None) -> XetFileData | None:
    ...
    if response is None:
        return None
    try:
        file_hash = response.headers[constants.MEGA_HEADER_X_XET_HASH]

        if constants.MEGA_HEADER_LINK_XET_AUTH_KEY in response.links:
            refresh_route = response.links[constants.MEGA_HEADER_LINK_XET_AUTH_KEY]["url"]
        else:
            refresh_route = response.headers[constants.MEGA_HEADER_X_XET_REFRESH_ROUTE]
    except KeyError:
        return None
    endpoint = endpoint if endpoint is not None else constants.ENDPOINT
    if refresh_route.startswith(constants.MEGA_URL_HOME):
        refresh_route = refresh_route.replace(constants.MEGA_URL_HOME.rstrip("/"), endpoint.rstrip("/"))
    return XetFileData(
        file_hash=file_hash,
    ...
    def get(self):
        ...
        with self._lock:
            ...
            if self._session is None:
                _configure_xet_upload_concurrency()
...
def get_xet_session():
    ...
    client returned by :func:`~megatensors._hub.utils._http.get_session` is shared.
