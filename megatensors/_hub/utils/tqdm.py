...
"""Utility helpers to handle progress bars in `megatensors._hub`.

Example:
    1. Use `megatensors._hub.utils.tqdm` as you would use `tqdm.tqdm` or `tqdm.auto.tqdm`.
    2. To disable progress bars, either use `disable_progress_bars()` helper or set the
       environment variable `MEGA_HUB_DISABLE_PROGRESS_BARS` to 1.
    3. To re-enable progress bars, use `enable_progress_bars()`.
    4. To check whether progress bars are disabled, use `are_progress_bars_disabled()`.

NOTE: Environment variable `MEGA_HUB_DISABLE_PROGRESS_BARS` has the priority.

Example:
    ```py
    >>> from megatensors._hub.utils import are_progress_bars_disabled, disable_progress_bars, enable_progress_bars, tqdm
...
from ..constants import MEGA_HUB_DISABLE_PROGRESS_BARS


# The `MEGA_HUB_DISABLE_PROGRESS_BARS` environment variable can be True, False, or not set (None),
...
#
# If `MEGA_HUB_DISABLE_PROGRESS_BARS` is not defined (None), it implies that users can manage
...
class disable_progress_bars:
    ...
    respects the `MEGA_HUB_DISABLE_PROGRESS_BARS` environment variable's setting.
    ...
    def __init__(self, name: str | None = None) -> None:
        ...
        if MEGA_HUB_DISABLE_PROGRESS_BARS is False:
            warnings.warn(
                "Cannot disable progress bars: environment variable `MEGA_HUB_DISABLE_PROGRESS_BARS=0` is set and has priority."
            )
...
def enable_progress_bars(name: str | None = None) -> None:
    ...
    if no group is specified. The operation is subject to the `MEGA_HUB_DISABLE_PROGRESS_BARS`
    environment setting.
    ...
    Raises:
        Warning: If the environment variable precludes changes.
    """
    if MEGA_HUB_DISABLE_PROGRESS_BARS is True:
        warnings.warn(
            "Cannot enable progress bars: environment variable `MEGA_HUB_DISABLE_PROGRESS_BARS=1` is set and has priority."
        )
        return
...
def are_progress_bars_disabled(name: str | None = None) -> bool:
    ...
    It checks the `MEGA_HUB_DISABLE_PROGRESS_BARS` environment variable first, then the programmatic
    settings.
...
def is_tqdm_disabled(log_level: int) -> bool | None:
    ...
    see https://github.com/huggingface/megatensors._hub/pull/2000 and https://github.com/huggingface/megatensors._hub/pull/2698.
    """
...
class tqdm(old_tqdm):
    ...
    def __delattr__(self, attr: str) -> None:
        """Fix for https://github.com/huggingface/megatensors._hub/issues/1603"""
        try:
...
# bar coordination on the MEGA subclass is not a supported use case. See #4065.
...
def tqdm_stream_file(path: Path | str) -> Iterator[io.BufferedReader]:
    ...
    First implemented in `transformers` in 2019 but removed when switched to git-lfs. Used in `megatensors._hub` to show
...
def _create_progress_bar(*, cls: type[old_tqdm], log_level: int, name: str | None = None, **kwargs) -> old_tqdm:
    ...
    (`MEGA_HUB_DISABLE_PROGRESS_BARS`, `disable_progress_bars()`, log level) and uses
    `disable=None` for TTY auto-detection (see https://github.com/huggingface/megatensors._hub/pull/2000),
    unless `TQDM_POSITION=-1` forces bars on (https://github.com/huggingface/megatensors._hub/pull/2698).
    ...
    Omits `name` which vanilla tqdm rejects with `TqdmKeyError`. See https://github.com/huggingface/megatensors._hub/issues/4050.
    """
    ...
    # MEGA subclass: keep the historical log-level / TTY behavior. Group-based
