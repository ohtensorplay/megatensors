...
_DSTORAGE_DLL_DIR_ENV_VAR = "MEGATENSORS_DSTORAGE_DLL_DIR"
_LEGACY_DSTORAGE_DOWNLOAD_ENV_VAR = "MEGATENSORS_DSTORAGE_NUPKG_URL"


def _get_dstorage_cache_dir() -> Path:
    return Path.home() / ".cache" / "megatensors"
...
def init_dstorage(device_id: int = 0) -> None:
    ...
    if not _inited_ds:
        ...
        status = megacpp.init_dstorage(device_id, 0, cudart_dll, str(_dstorage_dll_dir))
        if status != "ok":
...
class DStorageFileCopier(CopierInterface):
    ...
    def __init__(
        ...
        device: Device,
        stream_reader: megacpp.dstorage_stream_reader,
        ...
        self.fh: Any = None  # megacpp.dstorage_file_handle
    ...
    def submit_io(self, use_buf_register: bool, max_copy_block_size: int):
        ...
        self.fh = megacpp.dstorage_file_handle()
...
def new_dstorage_copier(device: Device, **kwargs) -> CopierConstructFunc:
    ...
    def construct(
        metadata: MegaTensorsMetadata, device: Device, framework: FrameworkOpBase
    ) -> CopierInterface:
