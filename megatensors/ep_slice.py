...
tensor), so a partial-read-capable loader can skip the unowned bytes:
    ...
    from megatensors.ep_slice import expert_parallel_filter

    loader = MegaTensorsFileLoader(pg, device, nogds=True)
...
def expert_parallel_filter(
    ...
    range. Pass the predicate to ``MegaTensorsFileLoader.set_tensor_filter`` or
...
def expert_parallel_filter_from_env() -> Optional[Callable[[str], bool]]:
    ...
    Recognized environment variables:
      ...
      ``MEGATENSORS_EP_SLICE=1``        enable EP-slice reading
      ``MEGATENSORS_EP_NUM_EXPERTS=N``  global routed-expert count (required)
      ``MEGATENSORS_EP_SIZE`` / ``_RANK``  override EP size/rank; otherwise
    ...
    """
    if os.getenv("MEGATENSORS_EP_SLICE", "0") != "1":
        return None
    num_experts = int(os.getenv("MEGATENSORS_EP_NUM_EXPERTS", "0"))
    if num_experts <= 0:
        return None
    ep_size = int(os.getenv("MEGATENSORS_EP_SIZE", "0"))
    ep_rank = int(os.getenv("MEGATENSORS_EP_RANK", "-1"))
