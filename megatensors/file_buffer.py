...
class FilesBufferOnDevice:
    r"""Device buffer for .megatensors files.
        ...
        Users should create this instance with MegaTensorsFileLoader.copy_files_to_device().
    ...
    def __init__(
        ...
        self.visible_tensor_counts: Dict[int, Dict[int, int]] = {}
        for rank, loaders in rank_loaders.items():
            self.instantiated[rank] = {}
            self.visible_tensor_counts[rank] = {}
            for lidx, loader in enumerate(loaders):
                ...
                self.visible_tensor_counts[rank][lidx] = visible_count
        self.pg = pg
    ...
    def _get_tensor(
        ...
        if self.auto_mem_delete:
            ...
            if (
                len(self.instantiated[rank][lidx])
                == self.visible_tensor_counts[rank][lidx]
            ):
