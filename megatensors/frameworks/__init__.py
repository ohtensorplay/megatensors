...
class FrameworkOpBase(ABC, Generic[T, K]):
    ...
    def get_storage_shape(
        ...
        the MEGA shape counts logical values while DLPack / PyTorch work in
        bytes, so the shape must be collapsed to a flat byte count.
        """
        return shape, strides

    def get_native_shape(self, dtype: DType, st_shape: List[int]) -> List[int]:
        """Return the framework-native tensor shape for a MEGA tensor.

        For most dtypes this matches *st_shape* exactly.  For packed sub-byte
        dtypes the MEGA shape counts logical (sub-byte) elements while
