...
"""Tests for megatensors.auto_loader module."""
...
from megatensors.auto_loader import _LOADER_REGISTRY, AutoLoader
from megatensors.config import CONFIG_ENV_VAR, LoaderConfig
from megatensors.loader import (
    BaseMegaTensorsFileLoader,
)
from megatensors.loader import MegaTensorsFileLoader as RealSafeLoader
from megatensors.parallel_loader import PipelineParallel
from megatensors.threefs_loader import ThreeFSLoader as RealThreeFSLoader
...
class TestAutoLoaderBaseLoader:
    """Test AutoLoader creates MegaTensorsFileLoader for loader='base'."""
    ...
    def test_default_creates_base_loader(self, monkeypatch):
        """Default config (loader='base') should create MegaTensorsFileLoader."""
        monkeypatch.delenv(CONFIG_ENV_VAR, raising=False)

        with (
            patch("megatensors.auto_loader.MegaTensorsFileLoader") as mock_stfl,
            patch("megatensors.auto_loader.PipelineParallel"),
            patch("megatensors.auto_loader.load_config") as mock_load,
        ):
            ...
            unified = AutoLoader(None, ["file1.megatensors"], device="cpu")
    ...
    def test_gds_creates_base_loader_with_nogds_false(self, monkeypatch):
        ...
        with (
            ...
            AutoLoader(None, ["file1.megatensors"], device="cuda:0")
    ...
    def test_base_no_extension_uses_defaults(self, monkeypatch):
        ...
        with (
            ...
            AutoLoader(None, ["file1.megatensors"], device="cpu")
...
class TestAutoLoader3FSLoader:
    ...
    def test_3fs_creates_threefs_loader(self, monkeypatch):
        ...
        with (
            patch("megatensors.auto_loader.ThreeFSLoader") as mock_3fs,
            patch("megatensors.auto_loader.PipelineParallel"),
            patch("megatensors.auto_loader.load_config") as mock_load,
        ):
            ...
            unified = AutoLoader(None, ["file1.megatensors"], device="cuda:0")
...
class TestAutoLoaderPipelineCreation:
    ...
    def test_pipeline_receives_parallel_kwargs(self):
        ...
        with (
            patch("megatensors.auto_loader.MegaTensorsFileLoader") as mock_stfl,
            patch("megatensors.auto_loader.PipelineParallel") as mock_pp,
            patch("megatensors.auto_loader.load_config") as mock_load,
        ):
...
class TestPipelineParallelSingleProcess:
    ...
    def test_none_pg_is_treated_as_single_process(self):
        ...
        pipeline = PipelineParallel(None, loader, ["a.megatensors", "b.megatensors"])

        assert pipeline.weight_files_batches == [["a.megatensors"], ["b.megatensors"]]
...
class TestAutoLoaderConfigDiscovery:
    ...
    def test_env_var_config(self, monkeypatch):
        """MEGATENSORS_CONFIG env var should point to config file."""
        data = {
            "loader": "3fs",
        ...
        try:
            ...
            with (
                ...
                mock_3fs.return_value = MagicMock()
                unified = AutoLoader(None, ["file1.megatensors"])
                assert unified.config.loader == "3fs"
                assert (
    ...
    def test_no_config_uses_defaults(self, monkeypatch):
        ...
        monkeypatch.delenv(CONFIG_ENV_VAR, raising=False)
        # cd to a temp dir where no megatensors.json exists
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                with (
                    patch(
                        "megatensors.auto_loader.MegaTensorsFileLoader"
                    ) as mock_stfl,
                    patch("megatensors.auto_loader.PipelineParallel"),
                ):
                    mock_stfl.return_value = MagicMock()
...
class TestProcessExtensionConfig:
    ...
    def test_base_process_extension_config_called(self):
        """MegaTensorsFileLoader.process_extension_config should be called."""
        with (
            patch("megatensors.auto_loader.MegaTensorsFileLoader") as mock_stfl,
            patch("megatensors.auto_loader.PipelineParallel"),
            patch("megatensors.auto_loader.load_config") as mock_load,
...
class TestThreeFSProcessExtensionConfig:
    ...
    def test_explicit_mount_point_preserved(self):
        ...
        result = RealThreeFSLoader.process_extension_config(
            ...
            hf_weights_files=["/mnt/x/a.megatensors"],
        )
    ...
    def test_no_mount_point_with_files_triggers_inference(self):
        ...
        with patch(
            "megatensors.threefs_loader.extract_mount_point",
            create=True,
        ) as mock_extract:
            ...
            with patch.dict("sys.modules", {"fastsafetensor_3fs_reader": mock_module}):
                result = RealThreeFSLoader.process_extension_config(
                    {},
                    hf_weights_files=["/inferred/3fs/model.megatensors"],
                )
    ...
    def test_import_error_graceful_degradation(self):
        ...
        try:
            with patch.dict("sys.modules", {"fastsafetensor_3fs_reader": None}):
                ...
                result = RealThreeFSLoader.process_extension_config(
                    {},
                    hf_weights_files=["/mnt/3fs/model.megatensors"],
                )
...
class TestKwargsPassedToProcessExtensionConfig:
    ...
    def test_hf_weights_files_passed_to_base(self):
        ...
        with (
            ...
            AutoLoader(None, ["a.megatensors", "b.megatensors"], device="cpu")
    ...
    def test_hf_weights_files_passed_to_3fs(self):
        ...
        with (
            ...
            AutoLoader(None, ["/data/3fs/m1.megatensors"], device="cuda:0")

            assert "hf_weights_files" in received_kwargs
            assert received_kwargs["hf_weights_files"] == ["/data/3fs/m1.megatensors"]


class TestInitExports:
    """Verify loader/config module entrypoints."""

    def test_auto_loader_importable(self):
        """AutoLoader should be importable from its module."""
        from megatensors.auto_loader import AutoLoader as AL

        assert AL is not None
        assert AL is AutoLoader

    def test_load_config_importable(self):
        """LoaderConfig and load_config should be importable from config."""
        from megatensors.config import LoaderConfig as LC
        from megatensors.config import load_config as lc
...
class TestBaseProcessExtensionConfigKwargs:
    """Verify BaseMegaTensorsFileLoader.process_extension_config accepts **kwargs."""

    def test_accepts_extra_kwargs(self):
        """Should accept arbitrary kwargs without error."""
        result = BaseMegaTensorsFileLoader.process_extension_config(
            {"key1": "val1"},
            hf_weights_files=["a.megatensors"],
            some_other_kwarg=42,
        )
