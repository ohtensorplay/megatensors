...
from threefs.conftest import (
    get_device,
    load_megatensors_file,
    tensors_equal,
)

from megatensors import cpp as megacpp
from megatensors.common import SingleGroup
from megatensors.frameworks import FrameworkOpBase
...
def example_files(framework) -> List[str]:
    if framework.get_name() == "pytorch":
        files = [
            os.path.join(EXAMPLES_DIR, "a.megatensors"),
            os.path.join(EXAMPLES_DIR, "b.megatensors"),
        ]
    elif framework.get_name() == "paddle":
        files = [
            os.path.join(EXAMPLES_DIR, "a_paddle.megatensors"),
            os.path.join(EXAMPLES_DIR, "b_paddle.megatensors"),
        ]
    else:
...
def test_parallel_single_file(megacpp_log, example_files, framework):
    ...
    expected = load_megatensors_file(example_files[0], device, framework)
    loaded = {}
...
def test_parallel_multiple_files(megacpp_log, example_files, framework):
    from megatensors.threefs_loader import ParallelThreeFSLoader
    ...
    for f in example_files:
        all_expected.update(load_megatensors_file(f, device, framework))
...
def test_parallel_close_and_memory_release(megacpp_log, example_files, framework):
    from megatensors.threefs_loader import ParallelThreeFSLoader
...
def test_parallel_close_without_iterate(megacpp_log, example_files, framework):
    from megatensors.threefs_loader import ParallelThreeFSLoader
