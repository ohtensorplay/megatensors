...
import pytest
from threefs.conftest import get_device, load_megatensors_file

from megatensors import MegaTensorsMetadata
from megatensors.common import SingleGroup
from megatensors.copier import create_copier_constructor


def test_threefs_copier_registered(megacpp_log, framework):
...
def test_ThreeFSFileCopier(megacpp_log, input_files, framework):
    from megatensors.copier.threefs import (
        ThreeFSFileCopier,
    ...
    meta = MegaTensorsMetadata.from_file(input_file, framework)
    ...
    expected = load_megatensors_file(input_file, device, framework)
...
def test_ThreeFSLoader(megacpp_log, input_files, framework):
    from megatensors.threefs_loader import ThreeFSLoader
...
def test_ThreeFSLoader_multiple_files(megacpp_log, input_files, framework):
    from megatensors.threefs_loader import ThreeFSLoader
    ...
    for test_file in input_files:
        expected = load_megatensors_file(test_file, device, framework)
        for key, exp in expected.items():
            actual = bufs.get_tensor_wrapped(key)
            assert framework.is_equal(actual, exp), f"Tensor mismatch for key: {key}"
